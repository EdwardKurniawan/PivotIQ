import { buildDemoReportData, hydrateModelReportData, normalizeReportData, parseModelJson, slugify } from './report-data';
import { enrichReportLearningResources } from './course-catalog';
import { groundReportWithLiveJobMarket } from './job-gap-analysis';
...
- Treat decision_scope, domain_focus, and core_systems as high-signal context about the environment, stakes, specialty lane, and systems this user actually operates inside.
...
- Use decision scope, domain focus, and core systems to sharpen the role read so it feels grounded in the user's actual operating context.
...
- Use decision scope, domain focus, and core systems to keep pivot titles role-native and avoid drifting into generic or fantasy titles.
...
- The next title must be a believable promotion or expansion, not "AI-Enabled [current role]".
- Leverage opportunities must be specific to the function.
- Use decision scope, domain focus, and core systems to make the stay path feel native to the user's real environment.
...
function repairWeakPivots(reportData, { jobTitle, industry, selectedTaskLabels, intakeProfile, locale }) {
  ...
  const repairedPivots = reportData.pivots.map((pivot, index) => {
    ...
    const familyMismatch =
      ...
      !titleMatchesRoleFamily(pivot?.title, roleFamily);
    ...
    if (!titleIsWeak && !familyMismatch && !genericGaps) return pivot;
    ...
    return {
      ...
      skill_gaps: fallbackPivot.skill_gaps,
    };
  });
