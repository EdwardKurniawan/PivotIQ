...
const FULL_REPORT_SCHEMA_GUIDE = `{
  ...
  "profile": {
    ...
    "clarifiers": {
      ...
      "technical_capability": "no_code_only | advanced_spreadsheets | sql_bi | scripting_python | software_engineering | null",
      "salary_tolerance": "cannot_take_cut | up_to_10_percent | up_to_20_percent | flexible_for_right_move | null",
      "proof_state": "none | internal_project | dashboard_or_analysis | workflow_or_playbook | portfolio_or_case_study | null",
...
function buildUserMessage(jobTitle, industry, tasks, intakeProfile, locale, stage) {
  ...
  const requirements = stage === 'preview'
    ...
    : [
        ...
        'If technical_capability is limited, do not recommend a role or learning start that quietly assumes coding-heavy comfort.',
        'If salary_tolerance is strict, avoid treating long-reset or unclear-payoff pivots as easy recommendations.',
        'If proof_state is already strong, do not behave as if the user is starting from zero; upgrade the proof asset and roadmap accordingly.',
        ...
        'For every pivot, prefer concrete hard skills, platforms, and technical tooling when the target role actually depends on them. Do not hide behind soft skills if the real hiring bar includes stack fluency.',
...
- Treat goal_now, timeline_urgency, years_experience_band, location_preference, ai_maturity, technical_capability, salary_tolerance, proof_state, decision_scope, domain_focus, and core_systems as high-signal context about the user\'s constraints, seniority, market, specialty lane, and operating environment.
...
- Use goal_now, timeline_urgency, years_experience_band, location_preference, ai_maturity, technical_capability, salary_tolerance, proof_state, decision scope, domain focus, and core systems to sharpen the role read so it feels grounded in the user's actual operating context.
...
- Use goal_now, timeline_urgency, years_experience_band, location_preference, ai_maturity, technical_capability, salary_tolerance, proof_state, decision scope, domain focus, and core systems to keep pivot titles role-native and avoid drifting into generic or fantasy titles.
- Each pivot needs concrete role-specific skill gaps with direct learning destination URLs.
- When the target role depends on stack fluency, name the real hard skills, tools, platforms, and systems a hiring manager would look for, not just softer communication or generic AI skills.
...
- Use goal_now, timeline_urgency, years_experience_band, location_preference, ai_maturity, technical_capability, salary_tolerance, proof_state, decision scope, domain focus, and core systems to make the stay path feel native to the user's real environment.
- Stay-path skill gaps should include real hard skills, systems, platform fluency, and workflow tooling when those are part of succeeding in the stronger version of the role.
...
function sanitizePivotSkillGaps(skillGaps, pivot, roleFamily) {
  const seen = new Set();
  return normalizeArray(skillGaps).filter((skillGap) => {
    if (!skillGap || !skillGap.skill_name) return false;
    if (isGenericAIGap(skillGap) && !pivotAllowsGenericAIGaps(pivot)) return false;
    if (isOffFamilySkillGap(skillGap, roleFamily)) return false;
    const key = normalizeText(skillGap.skill_name);
...
function repairWeakPivots(reportData, { jobTitle, industry, selectedTaskLabels, intakeProfile, locale }) {
  ...
  const repairedPivots = reportData.pivots.map((pivot, index) => {
    ...
    const offFamilyGaps = Array.isArray(pivot?.skill_gaps)
      ...
      : false;
    const sanitizedSkillGaps = sanitizePivotSkillGaps(pivot?.skill_gaps, pivot, roleFamily);
...
function stabilizeSparseCoveragePivots(reportData, { jobTitle, industry, selectedTaskLabels, intakeProfile, locale }) {
  ...
  const stabilizedPivots = reportData.pivots.map((pivot, index) => {
    ...
    return {
      ...
      difficulty: fallbackPivot.difficulty,
      strengths_to_leverage: fallbackPivot.strengths_to_leverage,
      skill_gaps: sanitizePivotSkillGaps(pivot?.skill_gaps, pivot, roleFamily),
