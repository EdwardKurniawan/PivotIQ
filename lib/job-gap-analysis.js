...
function scoreOpeningForPivot(opening, pivotTitle, roleFamily) {
  ...
  let score = overlapCount(titleTokens, openingTitleTokens) * 6;
  ...
  if (roleFamily && normalizeText(opening.role_family) === normalizeText(roleFamily)) score += 4;
...
export async function groundReportWithLiveJobMarket(reportData, fallbackProfile = null) {
  ...
  for (const pivot of reportData.pivots || []) {
    const marketRows = normalizeText(pivot?.role_family)
      ? allMarketRows.filter((opening) => normalizeText(opening.role_family) === normalizeText(pivot.role_family))
      : allMarketRows;
    const scored = marketRows
      .map((opening) => ({ opening, score: scoreOpeningForPivot(opening, pivot.title, pivot.role_family) }))
    ...
    const groundedMatchScore = market.openings_count
      ? Math.max(40, Math.min(95, Math.round((Number(pivot.match_score || 0) * 0.65) + (comparison.profile_fit_score * 0.35))))
      : pivot.match_score;
    ...
    enrichedPivots.push({
      ...pivot,
      match_score: groundedMatchScore,
      live_market_signal: marketSignal,
  ...
  return {
    ...reportData,
    pivots: enrichedPivots,
    live_market_grounding: {
      generated_at: new Date().toISOString(),
      current_profile_skills: profileSkills.current_skills,
      current_profile_tools: profileSkills.current_tools,
      pivots: pivotGrounding,
    },
  };
}
...
export async function analyzeProfileAgainstTargetRole({ profile, targetTitle, roleFamily = '' }) {
  ...
  const profileSkills = inferCurrentProfileSkills(profile || {}, selectedTaskLabels);
  const allMarketRows = await fetchOpenMarketRows();
  const marketRows = roleFamily
    ? allMarketRows.filter((opening) => normalizeText(opening.role_family) === normalizeText(roleFamily))
    : allMarketRows;
  const scored = marketRows
    .map((opening) => ({ opening, score: scoreOpeningForPivot(opening, targetTitle, roleFamily) }))
    .filter((item) => item.score >= 4)
    .sort((left, right) => right.score - left.score)
    .slice(0, 25)
    .map((item) => item.opening);
