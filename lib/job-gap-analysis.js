...
function rankPivotWithMarketSignal(pivot, signal) {
  ...
  if (!openings) {
    const fallbackScore = Math.max(
      30,
      Math.min(92, Math.round((originalMatch * 0.78) + (profileFit * 0.12) - modelOnlyPenalty))
    );
...
export async function groundReportWithLiveJobMarket(reportData, fallbackProfile = null) {
  ...
  for (const pivot of reportData.pivots || []) {
    ...
    const ranking = rankPivotWithMarketSignal(pivot, marketSignal);
