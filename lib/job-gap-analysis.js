...
const ROLE_PURITY_RULES = {
  ...
  operations: {
    required: [/\boperations\b/, /\bprogram\b/, /\bproject\b/, /\bdelivery\b/, /\bpmo\b/, /\bimplementation\b/],
    blocked: [/\blegal counsel\b/, /\bprocurement specialist\b/, /\binstructional\b/, /\bcurriculum\b/, /\brecruit/i],
  },
  hr: {
    required: [/\bpeople\b/, /\bhr\b/, /\bhuman resources\b/, /\btalent\b/, /\bemployee\b/, /\bworkforce\b/],
    blocked: [/\bcontract\b/, /\bclause\b/, /\bsupplier\b/, /\bprocurement\b/, /\bpricing\b/],
  },
  marketing: {
    required: [/\bmarketing\b/, /\bgrowth\b/, /\bcampaign\b/, /\bbrand\b/, /\bgtm\b/, /\bdemand gen\b/, /\blifecycle\b/],
    blocked: [/\bcontract\b/, /\bclause\b/, /\blegal\b/, /\bprocurement\b/, /\brecruit/i],
  },
  customer: {
    required: [/\bcustomer success\b/, /\bcustomer operations\b/, /\baccount management\b/, /\brenewal\b/, /\bretention\b/, /\bonboarding\b/, /\bservice\b/],
    blocked: [/\bcontract\b/, /\bclause\b/, /\blegal\b/, /\bprocurement\b/, /\brecruit/i],
  },
  analytics: {
    required: [/\bdata\b/, /\banalytics\b/, /\bbusiness intelligence\b/, /\breporting\b/, /\binsights\b/, /\bdashboard\b/, /\bpower bi\b/, /\btableau\b/, /\blooker\b/, /\bsql\b/],
    blocked: [/\bcontract\b/, /\bclause\b/, /\blegal\b/, /\bprocurement\b/, /\bcurriculum\b/, /\brecruit/i],
  },
};
...
const ROLE_FAMILY_DOMAIN_TOKENS = {
  ...
  operations: new Set(['operations', 'program', 'project', 'delivery', 'execution', 'pmo', 'implementation']),
  hr: new Set(['people', 'hr', 'talent', 'employee', 'workforce', 'human']),
  marketing: new Set(['marketing', 'growth', 'campaign', 'brand', 'gtm', 'lifecycle', 'demand', 'gen']),
  customer: new Set(['customer', 'success', 'renewal', 'account', 'retention', 'onboarding', 'service']),
  analytics: new Set(['data', 'analytics', 'business', 'intelligence', 'reporting', 'insights', 'dashboard', 'sql', 'bi']),
};
...
const SPECIALIZED_ROLE_TITLE_PATTERNS = {
  ...
  finance: /\b(finance|financial|fp&a|fpa|forecast|budget|pricing|commercial finance)\b/,
  operations: /\b(operations|program|project|delivery|pmo|implementation|change management)\b/,
  hr: /\b(people|hr|human resources|talent|employee|workforce)\b/,
};
...
const ROLE_NATIVE_SKILL_PATTERNS = {
  ...
  finance: /\b(finance|financial|forecast|budget|variance|pricing|modeling|planning|fp&a|sql|power bi|erp|scenario|commercial)\b/i,
  operations: /\b(program|project|operations|workflow|delivery|pmo|change|coordination|execution|automation|integration|handoff|jira|airtable)\b/i,
  hr: /\b(people|hr|employee|talent|workforce|policy|coaching|org)\b/i,
  marketing: /\b(marketing|growth|campaign|brand|gtm|launch|lifecycle|crm|attribution|demand gen|experimentation|analytics|hubspot|marketo)\b/i,
  customer: /\b(customer|renewal|retention|account|customer success|customer operations|health score|crm|salesforce|gainsight|service)\b/i,
  analytics: /\b(data|analytics|dashboard|kpi|sql|python|business intelligence|reporting|insights|snowflake|bigquery|dbt|etl|elt|warehouse|tableau|looker|power bi|data model|data pipeline)\b/i,
};

const GENERIC_MARKET_TOOLS = /\b(excel|google sheets|slack|zoom|microsoft office|office|notion|jira|email|outlook|docs)\b/i;

const FAMILY_HARD_SKILL_PATTERNS = {
  analytics: /\b(sql|python|snowflake|bigquery|redshift|dbt|etl|elt|data warehouse|warehouse|tableau|looker|power bi|data model|semantic layer|data pipeline|database architecture)\b/i,
  finance: /\b(sql|power bi|excel|spreadsheet|scenario model|financial model|erp|netsuite|sap|planning model|forecast model)\b/i,
  operations: /\b(automation|integration|api|workflow|handoff|power automate|zapier|airtable|servicenow)\b/i,
  customer: /\b(salesforce|hubspot|gainsight|crm|health score|segmentation|renewal model|workflow automation)\b/i,
  marketing: /\b(hubspot|marketo|google analytics|ga4|attribution|lifecycle|crm|experimentation|a\/b testing|campaign automation)\b/i,
  hr: /\b(workday|hris|people analytics|workflow automation|employee data)\b/i,
  legal: /\b(clm|contract lifecycle|clause library|approval routing|privacy workflow)\b/i,
  procurement: /\b(spend analytics|supplier scorecard|erp|sourcing workflow|vendor data)\b/i,
  education: /\b(lms|curriculum ops|learning analytics|onboarding workflow)\b/i,
};
...
function titleMatchesCanonicalSpecializedFamily(title, roleFamily) {
  ...
  if (normalizedFamily === 'finance') {
    return /\b(finance business partner|strategic finance analyst|commercial finance analyst|fp&a manager|finance systems manager|pricing strategy manager|revenue planning manager)\b/.test(normalizedTitle);
  }

  if (normalizedFamily === 'operations') {
    return /\b(program operations manager|project operations manager|delivery operations manager|change management lead|pmo manager|portfolio operations manager|workflow operations manager)\b/.test(normalizedTitle);
  }

  if (normalizedFamily === 'hr') {
    return /\b(people operations manager|people operations lead|workforce planning manager|talent operations manager|manager enablement lead|hr operations manager)\b/.test(normalizedTitle);
  }
...
function inferPivotRoleFamily(pivot) {
  ...
  if (/(data analyst|business intelligence|analytics|reporting analyst|power bi|tableau|looker|sql analyst|insights)/.test(normalized)) return 'analytics';
  if (/(operations|program|project|delivery|implementation|pmo|change management)/.test(normalized)) return 'operations';
  if (/(people|hr|human resources|talent|employee|workforce)/.test(normalized)) return 'hr';
  return '';
}
...
function inferProfileRoleFamily(profile) {
  ...
  if (/(finance|financial|fp&a|fpa|forecast|budget|pricing|commercial finance)/i.test(text)) return 'finance';
  if (/(marketing|growth|brand|gtm|go to market|product marketing|demand gen|campaign)/i.test(text)) return 'marketing';
  if (/(customer success|account management|customer operations|renewal|retention|customer education|service delivery)/i.test(text)) return 'customer';
  if (/(data analyst|business intelligence|analytics|reporting analyst|power bi|tableau|looker|sql analyst|insights)/i.test(text)) return 'analytics';
  if (/(project manager|program manager|project management|program management|pmo|delivery|implementation|change management|operations)/i.test(text)) return 'operations';
  if (/(hr|human resources|people operations|people partner|talent|workforce|employee relations|recruiter)/i.test(text)) return 'hr';
  return '';
}

function inferSkillsFromTechnicalCapability(capability) {
  const normalized = normalizeText(capability);
  if (normalized === 'advanced spreadsheets') return ['Spreadsheet Modeling', 'Scenario Analysis'];
  if (normalized === 'sql bi') return ['SQL', 'Dashboarding', 'Business Intelligence'];
  if (normalized === 'scripting python') return ['SQL', 'Python', 'Workflow Automation'];
  if (normalized === 'software engineering') return ['Python', 'API Integration', 'Systems Design'];
  return [];
}

function inferSkillsFromSystems(coreSystems) {
  const skills = [];
  for (const system of coreSystems) {
    const normalized = normalizeText(system);
    if (/sql/.test(normalized)) skills.push('SQL');
    if (/\bpython\b/.test(normalized)) skills.push('Python');
    if (/power bi|tableau|looker|metabase/.test(normalized)) skills.push('BI Dashboarding');
    if (/snowflake|bigquery|redshift|databricks|data warehouse/.test(normalized)) skills.push('Data Warehouse');
    if (/dbt|etl|elt|fivetran|airbyte|stitch/.test(normalized)) skills.push('ETL');
    if (/salesforce|hubspot|gainsight/.test(normalized)) skills.push('CRM Operations');
    if (/excel|google sheets/.test(normalized)) skills.push('Spreadsheet Modeling');
    if (/netsuite|sap|oracle|erp/.test(normalized)) skills.push('ERP Data Workflows');
    if (/workday|hris/.test(normalized)) skills.push('People Systems');
    if (/clm|ironclad|contract/.test(normalized)) skills.push('CLM Systems');
  }
  return skills;
}
...
function inferCurrentProfileSkills(profile, selectedTaskLabels = []) {
  const clarifiers = profile?.clarifiers || {};
  ...
  const technicalCapability = clarifiers?.technical_capability || profile?.technical_capability || '';
  skills.push(...inferSkillsFromTechnicalCapability(technicalCapability));

  const coreSystems = normalizeStringList(clarifiers?.core_systems || profile?.core_systems);
  skills.push(...inferSkillsFromSystems(coreSystems));
  const tools = unique(coreSystems, 10);

  if (clarifiers?.management_scope && clarifiers.management_scope !== 'none') {
    skills.push('People Management');
  }
  if (clarifiers?.role_blend === 'strategy') {
...
function estimateSparseCoverageRealismPenalty(pivot, profileRoleFamily, marketSignal) {
  ...
  } else if (normalizedFamily === 'finance') {
    if (/\b(strategist|systems lead|transformation|commercial growth|pricing optimization|ai program)\b/.test(title)) penalty += 10;
    if (/[–—-]/.test(title)) penalty += 4;
  } else if (normalizedFamily === 'operations') {
    if (/\b(director|strategic execution|architect|consultant)\b/.test(title)) penalty += 10;
    if (/[–—-]/.test(title)) penalty += 4;
  } else if (normalizedFamily === 'hr') {
    if (/\b(ai program|contract|legal|architect)\b/.test(title)) penalty += 10;
    if (/[–—-]/.test(title)) penalty += 4;
  }
  ...
  const preferredPattern = normalizedFamily === 'legal'
    ...
    : normalizedFamily === 'education'
      ...
      : normalizedFamily === 'procurement'
        ...
        : normalizedFamily === 'finance'
          ? /\b(finance business partner|strategic finance analyst|commercial finance analyst|fp&a manager|finance systems manager|pricing strategy manager|revenue planning manager)\b/
          : normalizedFamily === 'operations'
            ? /\b(program operations manager|project operations manager|delivery operations manager|change management lead|pmo manager|portfolio operations manager|workflow operations manager)\b/
            : normalizedFamily === 'hr'
              ? /\b(people operations manager|people operations lead|workforce planning manager|talent operations manager|manager enablement lead|hr operations manager)\b/
        : null;
...
function roleNativeSkillScore(skillGap, profileRoleFamily, marketSignal) {
  ...
  const hardSkillPattern = FAMILY_HARD_SKILL_PATTERNS[normalizedFamily];
  ...
  if (hardSkillPattern && hardSkillPattern.test(normalizeText([skillGap?.skill_name, skillGap?.category, skillGap?.how_to_close_gap].join(' ')))) score += 14;
  ...
  if (/(technical stack|platform fluency|technical analytics|data stack)/i.test(String(skillGap?.category || ''))) score += 6;
...
function isOffFamilyRoleSkillGap(skillGap, profileRoleFamily) {
  ...
  if (normalizedFamily === 'finance') return /\b(contract|clause|legal intake|curriculum|instructional|customer success|sales enablement|recruiting|candidate)\b/.test(normalizedSkill);
  if (normalizedFamily === 'operations') return /\b(contract clause|clause library|curriculum|instructional|clinical|legal review|procurement sourcing)\b/.test(normalizedSkill);
  if (normalizedFamily === 'hr') return /\b(contract clause|clause library|legal intake|clm|sourcing|supplier|forecasting model|pricing strategy)\b/.test(normalizedSkill);
  return false;
}

function prioritizeRoleNativeSkillGaps(skillGaps, profileRoleFamily, marketSignal) {
  const normalizedFamily = normalizeText(profileRoleFamily);
  if (!Array.isArray(skillGaps) || !skillGaps.length || !ROLE_NATIVE_SKILL_PATTERNS[normalizedFamily]) return skillGaps;
...
function buildMarketGapSkill(skillLabel, count, pivot, profile) {
  ...
  const category = /(sql|python|snowflake|bigquery|redshift|dbt|etl|elt|tableau|looker|power bi|data model|database|warehouse)/.test(normalizedLabel)
    ? 'technical stack'
    : /(salesforce|hubspot|gainsight|marketo|workday|netsuite|sap|erp|crm|clm)/.test(normalizedLabel)
      ? 'platform fluency'
      : 'market demand';
  return {
    ...
    how_to_close_gap: `Build one proof asset that uses ${skillLabel} on a realistic business problem tied to ${roleName}, then show the workflow and business output clearly.`,
    market_backed: true,
    market_demand_count: count,
  };
}

function buildMarketToolGapSkill(toolLabel, count, pivot, profile) {
  const roleName = pivot?.title || 'this role';
  const currentTitle = profile?.job_title || 'your current role';
  return {
    skill_name: toolLabel,
    category: 'platform fluency',
    current_strength: `You already have adjacent workflow context from ${currentTitle}, but this platform is not yet clearly signaled in your current profile.`,
    required_level: `Comfortable enough to use ${toolLabel} in one credible workflow, proof asset, or systems review for ${roleName}.`,
    gap_priority: count >= 3 ? 'critical' : 'medium',
    why_it_matters: `${toolLabel} keeps appearing in live postings for ${roleName}, so platform fluency is part of the real bar for this move.`,
    evidence_you_already_have: 'You already understand the business questions around this work, which makes the platform ramp more believable.',
    how_to_close_gap: `Rebuild one recurring task or proof asset using ${toolLabel} so the platform shows up in visible work, not just in learning notes.`,
    market_backed: true,
    market_demand_count: count,
    tool_backed: true,
  };
...
function skillGapMatchesMarketSignal(skillGap, signalLabels) {
  const skillText = normalizeText([skillGap?.skill_name, skillGap?.category, skillGap?.how_to_close_gap].join(' '));
  if (!skillText) return false;
  return (signalLabels || []).some((label) => {
    const normalizedLabel = normalizeText(label);
    return normalizedLabel && (skillText.includes(normalizedLabel) || normalizedLabel.includes(skillText));
  });
}
...
function dedupeSkillGaps(skillGaps) {
  ...
  for (const gap of skillGaps || []) {
    const key = normalizeSkill(gap?.skill_name);
...
function marketSignalPriority(label, count, roleFamily, type = 'skill') {
  const normalizedFamily = normalizeText(roleFamily);
  const labelText = normalizeText(label);
  const nativePattern = ROLE_NATIVE_SKILL_PATTERNS[normalizedFamily];
  const hardSkillPattern = FAMILY_HARD_SKILL_PATTERNS[normalizedFamily];

  let score = Number(count || 0) * 3;
  if (hardSkillPattern && hardSkillPattern.test(labelText)) score += 18;
  if (nativePattern && nativePattern.test(labelText)) score += 10;
  if (type === 'tool' && /(snowflake|bigquery|redshift|dbt|tableau|looker|power bi|salesforce|hubspot|workday|sap|netsuite)/.test(labelText)) score += 8;
  if (/\b(stakeholder management|written communication|communication|data analysis|analysis|ai tooling|program management)\b/.test(labelText)) score -= 8;
  return score;
}
...
function compareModelAndMarket({ pivot, market, profileSkills }) {
  ...
  const missingTools = marketTools.filter((tool) => !currentToolSet.has(normalizeSkill(tool)) && !currentSkillSet.has(normalizeSkill(tool)) && !GENERIC_MARKET_TOOLS.test(tool));
  ...
  const marketOnlyTools = missingTools.filter((tool) => !modelGapNames.some((gap) => normalizeSkill(gap) === normalizeSkill(tool)));
  ...
  return {
    ...
    missing_market_tools: missingTools,
    market_only_tools: marketOnlyTools,
...
function mergeMarketSkillGaps(pivot, comparison, market, profile) {
  ...
  const gapRoleFamily = inferPivotRoleFamily(pivot) || inferProfileRoleFamily(profile || {});
  const marketOnly = [...comparison.market_only_required_skills]
    .sort((left, right) => (
      marketSignalPriority(right, market.required_skills.find((item) => item.label === right)?.count || 1, gapRoleFamily)
      - marketSignalPriority(left, market.required_skills.find((item) => item.label === left)?.count || 1, gapRoleFamily)
    ))
    .map((skill) => {
    ...
    .slice(0, 4);
  const marketOnlyTools = [...comparison.market_only_tools]
    .sort((left, right) => (
      marketSignalPriority(right, market.tools.find((item) => item.label === right)?.count || 1, gapRoleFamily, 'tool')
      - marketSignalPriority(left, market.tools.find((item) => item.label === left)?.count || 1, gapRoleFamily, 'tool')
    ))
    .map((tool) => {
      const count = market.tools.find((item) => item.label === tool)?.count || 1;
      return buildMarketToolGapSkill(tool, count, pivot, profile || {});
    })
    .slice(0, 3);

  const normalizedMarket = new Set(comparison.market_required_skills.map(normalizeSkill));
  const normalizedTools = new Set(comparison.market_tools.map(normalizeSkill));
  ...
  const enhancedExisting = existing.map((gap) => ({
    ...gap,
    market_backed: skillGapMatchesMarketSignal(gap, [...comparison.market_required_skills, ...comparison.market_tools]) || normalizedMarket.has(normalizeSkill(gap?.skill_name)),
    tool_backed: skillGapMatchesMarketSignal(gap, comparison.market_tools) || normalizedTools.has(normalizeSkill(gap?.skill_name)),
    already_signaled_by_user: normalizedCurrent.has(normalizeSkill(gap?.skill_name)),
  }));

  const combined = dedupeSkillGaps([
    ...enhancedExisting.sort((left, right) => {
      const leftScore = (left.market_backed ? 2 : 0) + (left.tool_backed ? 1 : 0) + (left.gap_priority === 'critical' ? 1 : 0);
      const rightScore = (right.market_backed ? 2 : 0) + (right.tool_backed ? 1 : 0) + (right.gap_priority === 'critical' ? 1 : 0);
  ...
  return prioritizeRoleNativeSkillGaps(combined, gapRoleFamily, pivot?.live_market_signal || {
...
export async function groundReportWithLiveJobMarket(reportData, fallbackProfile = null) {
  ...
  for (const pivot of reportData.pivots || []) {
    ...
    const marketSignal = {
      ...
      missing_market_tools: comparison.missing_market_tools,
      market_only_tools: comparison.market_only_tools,
    ...
    enrichedPivots.push({
      ...
      ranking_reason: ranking.ranking_reason,
      original_match_score: ranking.original_match_score,
      skill_gaps: prioritizeRoleNativeSkillGaps(mergedSkillGaps, pivotRoleFamily || profileRoleFamily, enrichedMarketSignal),
    });
