...
function estimateSpecializedTitlePenalty(pivot, pivotRoleFamily, profileRoleFamily, marketSignal) {
  ...
  const titleMatchesProfile = titleMatchesSpecializedFamily(pivot?.title, normalizedProfileFamily);
...
export function rankPivotWithMarketSignal(pivot, signal, profile = null) {
  ...
  if (!openings) {
    const fallbackScore = Math.max(
      28,
      Math.min(40, Math.round(28 + (originalMatch * 0.14) - modelOnlyPenalty - seniorityPenalty - specializedTitlePenalty - specializedAiPrefixPenalty - noOpeningFramePenalty))
    );
