...
export function classifyJobTrack(job = {}) {
  ...
  if (/\bengineer\b|\bengineering\b|developer|software|full stack|backend|frontend|\bapi\b|\binfrastructure\b|\bplatform\b|\barchitect\b|\barchitecture\b/.test(text)) return 'engineering';
  ...
  if (/\benablement\b|\bonboarding\b|\binstructional\b|\beducation\b|\blearning\b(?!\s+(infrastructure|engineer|engineering|platform|systems?|model))/.test(text)) return 'education';
...
export function isGroundingEligibleForPivot(opening = {}, pivotTitle = '', roleFamily = '') {
  ...
  if (track === 'engineering' && family && family !== 'engineering') {
    return false;
  }

  if (track === 'recruiting' && family && family !== 'general') {
    return false;
  }
