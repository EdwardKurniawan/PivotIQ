...
function repairLowerPivotFamilies(reportData, warnings, repairs) {
  const family = inferSpecializedFamily(reportData);
  if (!family || !['customer', 'finance', 'marketing', 'analytics'].includes(family)) return reportData;
  ...
  const pivots = normalizeArray(reportData.pivots).map((pivot, index) => {
    ...
    if (
      family === 'customer'
      && weakSignal
      && /\b(product|platform|head|director|data analyst|revenue operations)\b/.test(normalizedTitle)
    ) {
      shouldRepair = true;
    }
    ...
    return {
      ...
      skill_gaps: normalizeArray(pivot.skill_gaps).map((skill, skillIndex) => (
        skillIndex === 0 && isOffFamilySkillForFamily(skill, family)
