...
function cleanSkillGap(skill, warnings, repairs) {
  ...
  if (cleanedName && cleanedName !== nextSkill.skill_name) {
    warnings.push(`Simplified overlong skill gap name: ${nextSkill.skill_name || 'unknown skill'}.`);
...
function repairTopPivotFamily(reportData, warnings, repairs) {
  ...
  if (!family || titleMatchesFamily(reportData?.pivots?.[0]?.title, family)) return reportData;

  const replacementIndex = normalizeArray(reportData.pivots).findIndex((pivot) => titleMatchesFamily(pivot?.title, family));
...
export function applyReportQualityGate(reportData) {
  ...
  if (!first30DaysMentionsPivot(nextReport.first_30_days, finalTopPivotAfterSkillRepair?.title)) {
    nextReport = {
      ...nextReport,
      first_30_days: buildFirst30DaysFallback(nextReport, finalTopPivotAfterSkillRepair),
    };
    repairs.push('Rebuilt first 30 days to match the final active pivot.');
  }
