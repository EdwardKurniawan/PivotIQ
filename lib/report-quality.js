...
function titleMatchesCanonicalFamily(title, family) {
  ...
  if (family === 'finance') return /\b(finance business partner|strategic finance analyst|commercial finance analyst|fp&a manager|finance systems manager|pricing strategy manager|revenue planning manager)\b/.test(normalized);
  ...
  if (family === 'customer') return /\b(customer strategy lead|customer operations lead|customer success director|customer success manager|account strategy lead|customer enablement lead)\b/.test(normalized);
  return true;
}
