...
function titleMatchesCanonicalFamily(title, family) {
  ...
  if (family === 'customer') return /\b(customer strategy lead|customer success strategy lead|customer success strategy manager|customer operations lead|customer success director|customer success manager|account strategy lead|customer enablement lead)\b/.test(normalized);
...
function repairFamilyTopSkill(reportData, family, warnings, repairs) {
  ...
  if (shouldRepairTopBundleForFamily(topPivot?.skill_gaps, family)) {
    const bundle = buildRoleNativeTopBundle(family);
    if (bundle.length) {
      repairs.push(`Replaced ${family} top-pivot skills with a more role-native learning bundle.`);
      const repairedReport = {
        ...reportData,
        pivots: reportData.pivots.map((pivot, pivotIndex) => {
          if (pivotIndex !== 0) return pivot;
          return {
            ...
            skill_gaps: [
              ...bundle,
              ...normalizeArray(pivot.skill_gaps).slice(bundle.length),
            ],
          };
        }),
      };
...
function buildFamilyRepairSkill(skill, family) {
  ...
  if (family === 'customer') {
    return {
      ...
      why_it_matters: 'Customer-success pivots need evidence that you can turn account signals into earlier renewal and expansion decisions.',
      how_to_close_gap: 'Build one account health and renewal review workflow with flags, owners, and action triggers.',
    };
  }
  if (family === 'marketing') {
    return {
      ...
      how_to_close_gap: 'Rebuild one campaign review into an experiment brief with success metrics, audience logic, and next-step decisions.',
    };
  }
  if (family === 'analytics') {
    return {
      ...
      why_it_matters: 'Analytics pivots get stronger when you can turn reporting into decisions, not just cleaner charts.',
      how_to_close_gap: 'Rebuild one dashboard into a decision review that ties metrics to actions, owners, and next questions.',
    };
  }
  return skill;
}
...
function isOffFamilySkillForFamily(skill, family) {
  ...
  if (family === 'operations') return /\b(clause library|contract review|curriculum|instructional|supplier sourcing|machine learning|statistical modeling)\b/.test(text);
  if (family === 'customer') return /\b(contract|clause|curriculum|python|sql|statistical modeling|machine learning)\b/.test(text);
  if (family === 'marketing') return /\b(contract|clause|curriculum|python|sql|machine learning)\b/.test(text);
...
function shouldRepairTopBundleForFamily(skillGaps, family) {
  ...
  if (family === 'customer') {
    return (
      ...
      !/\b(customer|renewal|account|retention|customer success|customer operations|service)\b/.test(nativeRoleSignals)
    );
  }
  if (family === 'marketing') {
    return (
      /\b(prompt design|sql|python|machine learning|data science|business process management)\b/.test(text) ||
      !/\b(marketing|campaign|growth|launch|gtm|brand)\b/.test(nativeRoleSignals)
    );
  }
  if (family === 'operations') {
    return (
      /\b(prompt design|machine learning|systems architecture|statistical modeling)\b/.test(text) ||
      !/\b(workflow|operations|process|delivery|program|project)\b/.test(nativeRoleSignals)
    );
  }
  if (family === 'hr') {
    return (
  ...
  if (family === 'finance') {
    return (
      ...
      !/\b(finance|financial|forecast|scenario|planning|budget|pricing|commercial)\b/.test(nativeRoleSignals)
    );
  }
  if (family === 'analytics') {
    return (
      ...
      !/\b(data|analytics|dashboard|kpi|sql|business intelligence|reporting|insights)\b/.test(nativeRoleSignals)
    );
  }
  return false;
}
