...
const SPECIFIC_RESOURCE_RULES = [
  ...
  { pattern: /\bmanager enablement\b|\bpeople operations\b|\bworkforce planning\b/i, resource_title: 'People operations systems paths', resource_url: 'https://www.coursera.org/search?query=people%20operations%20systems%20manager%20enablement', resource_provider: 'Coursera' },
  ...
  { pattern: /\bgrowth review operating system\b|\bmarketing automation governance\b|\battribution and lifecycle logic\b/i, resource_title: 'Marketing operations and lifecycle paths', resource_url: 'https://www.coursera.org/search?query=marketing%20operations%20lifecycle%20management', resource_provider: 'Coursera' },
  { pattern: /\bplanning review cadence\b|\bplanning model governance\b/i, resource_title: 'Finance planning operations paths', resource_url: 'https://www.coursera.org/search?query=finance%20planning%20operations%20cadence', resource_provider: 'Coursera' },
  { pattern: /\bmanager support operating standard\b/i, resource_title: 'People operations systems paths', resource_url: 'https://www.coursera.org/search?query=people%20operations%20systems%20manager%20enablement', resource_provider: 'Coursera' },
  { pattern: /\brenewal playbook governance\b|\bcustomer workflow orchestration\b/i, resource_title: 'Customer operations playbook paths', resource_url: 'https://www.udemy.com/courses/search/?q=customer%20success%20operations%20playbook', resource_provider: 'Udemy' },
  { pattern: /\bexception and handoff governance\b|\bteam workflow governance\b|\bexception routing logic\b/i, resource_title: 'Process governance and handoff design paths', resource_url: 'https://www.coursera.org/search?query=process%20governance%20handoff%20design', resource_provider: 'Coursera' },
  { pattern: /\bexecutive decision cadence\b|\bexecutive operating rhythm design\b/i, resource_title: 'Executive operations and chief of staff paths', resource_url: 'https://www.coursera.org/search?query=chief%20of%20staff%20executive%20operations', resource_provider: 'Coursera' },
  { pattern: /\brevenue operations\b|\brevops\b|\bfunnel reporting\b|\bcrm reporting\b/i, resource_title: 'Revenue operations skills', resource_url: 'https://academy.hubspot.com/search?search=revenue%20operations', resource_provider: 'HubSpot Academy' },
...
const EXPLICIT_TOOL_PATTERNS = [
  ['looker', /\blooker\b|\blookml\b/i],
  ['snowflake', /\bsnowflake\b/i],
  ['bigquery', /\bbigquery\b/i],
  ['dbt', /\bdbt\b/i],
  ['power bi', /\bpower bi\b|\bdax\b/i],
  ['power query', /\bpower query\b/i],
  ['tableau', /\btableau\b/i],
  ['sql', /\bsql\b/i],
  ['python', /\bpython\b/i],
  ['salesforce', /\bsalesforce\b|\bcrm analytics\b|\bservice cloud\b/i],
  ['hubspot', /\bhubspot\b|\bservice hub\b/i],
  ['gainsight', /\bgainsight\b/i],
  ['marketo', /\bmarketo\b/i],
  ['workday', /\bworkday\b|\bhris\b/i],
...
function inferExplicitTools(value) {
  const text = String(value || '');
  return EXPLICIT_TOOL_PATTERNS
...
function inferSkillIntent(skillGap) {
  ...
  if (/(governance|cadence|operating standard|operating system|playbook|handoff|review standards|decision cadence)/.test(text)) return 'governance';
...
function inferEntryIntent(entry) {
  ...
  if (/(governance|cadence|operating standard|operating system|playbook|handoff|review system|review rhythm|operating model)/.test(text)) return 'governance';
  if (/(stakeholder|communication|presentation|storytelling|narrative|executive|influence|alignment)/.test(text)) return 'communication';
  if (/(leadership|change management|facilitation|coaching|management)/.test(text)) return 'leadership';
  if (/(sql|python|dashboard|analysis|analytics|data|spreadsheet|modeling|query|database)/.test(text)) return 'technical';
  if (/(workflow|automation|process|operations|program|project)/.test(text)) return 'operations';
...
function inferDomainSignals(value) {
  ...
  if (/(revenue operations|revops|crm reporting|funnel reporting)/.test(normalized)) signals.push('revops');
...
function buildSpecificResourceFallback(skillGap, pivot) {
  ...
  if (
    ...
    /\b(financial modeling|scenario modeling|forecasting|planning model|planning workflow|budget|review cadence)\b/i.test(text)
  ) {
    return {
      resource_title: 'Finance planning operations paths',
      resource_url: 'https://www.coursera.org/search?query=finance%20planning%20operations%20cadence',
...
export function buildCatalogMatch(skillGap, pivot, catalogEntries) {
  ...
  const exactSkillPhrase = normalizePhrase(skillGap?.skill_name);
  const queryTools = inferExplicitTools([
    skillGap?.skill_name,
    skillGap?.how_to_close_gap,
    skillGap?.why_it_matters,
  ].join(' '));
  ...
  let best = null;
  const seniorityText = [pivot?.title, pivot?.decision_frame].join(' ').toLowerCase();
  const seniorityLevel = /\b(director|head|vp|vice president|chief)\b/.test(seniorityText)
    ? 3
    : /\b(manager|lead|partner)\b/.test(seniorityText)
      ? 2
      : 1;
  ...
  for (const entry of catalogEntries) {
    ...
    const titlePhrase = normalizePhrase(entry.title);
    const entryTools = inferExplicitTools([
      entry.title,
      entry.summary,
      Array.isArray(entry.skills) ? entry.skills.join(' ') : '',
      Array.isArray(entry.tags) ? entry.tags.join(' ') : '',
    ...
    if (exactSkillPhrase && exactSkillPhrase.length > 2) {
      if (titlePhrase.includes(exactSkillPhrase)) score += 16;
      else if (entrySearchText.includes(exactSkillPhrase)) score += 10;
    }

    if (queryTools.length) {
      const toolOverlap = queryTools.filter((tool) => entryTools.includes(tool)).length;
      if (toolOverlap) {
        score += toolOverlap * 18;
      } else if (entryTools.length) {
        score -= 12;
      }
    }
    ...
    if (
      roleSignals.includes('revops') &&
      /\b(revenue operations|revops|crm reporting|funnel|pipeline)\b/.test(entrySearchText)
    ) {
      score += 12;
    }
    ...
    if (skillIntent === 'governance' && entryIntent === 'governance') {
      score += 8;
    }

    if (skillIntent === 'governance' && entryIntent === 'technical' && directSkillOverlap === 0) {
      score -= 8;
    }

    if (skillIntent === 'governance' && entryIntent === 'communication' && directSkillOverlap === 0) {
      score -= 5;
    }
    ...
    if (
      seniorityLevel >= 2 &&
      /\b(essentials|fundamentals|foundations|beginner|introduction|intro)\b/.test(entrySearchText) &&
...
function inferRoleFamilies(value) {
  ...
  if (/(revenue operations|revops|crm reporting|funnel reporting|pipeline operations)/.test(normalized)) families.push('revops');
