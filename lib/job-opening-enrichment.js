...
${String(job.description_text || '').slice(0, 12000)}`;
  ...
  return {
    role_family: String(parsed.role_family || job.role_family || 'general').trim().toLowerCase(),
    domain_focus: String(parsed.domain_focus || job.domain_focus || '').trim(),
    seniority: normalizeSeniority(parsed.seniority || job.seniority),
    required_skills: unique(parsed.required_skills, 10),
    preferred_skills: unique(parsed.preferred_skills, 10),
    tools: unique(parsed.tools, 10),
    job_functions: unique(parsed.job_functions, 8),
