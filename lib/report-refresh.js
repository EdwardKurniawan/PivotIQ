...
function buildChangeDrivers(progressContext = {}, refreshedReportData = null) {
  ...
  if (progressContext?.completed_weeks_count) {
    drivers.push(`You completed ${progressContext.completed_weeks_count} roadmap milestone${progressContext.completed_weeks_count === 1 ? '' : 's'}, which gave PivotIQ stronger execution signal.`);
  }

  if (progressContext?.proof_ready_count) {
    drivers.push(`${progressContext.proof_ready_count} proof-ready milestone${progressContext.proof_ready_count === 1 ? '' : 's'} told PivotIQ you now have stronger visible evidence, not just intent.`);
  }

  if (progressContext?.manager_done_count) {
    drivers.push(`${progressContext.manager_done_count} manager conversation${progressContext.manager_done_count === 1 ? '' : 's'} showed that the plan is being tested in the real environment, not just planned privately.`);
  }
...
function buildRefreshOpportunitySummary({ isReady, proofReadyCount, managerDoneCount, completedWeeksCount, buildModeCount, meaningfulOutcome }) {
  ...
  if (proofReadyCount > 0 || managerDoneCount > 0) {
    return {
      title: 'Your proof and conversations can now sharpen the recommendation',
      body: 'You already logged signal that goes beyond planning. Refreshing now should tighten the recommendation, strengthen the action plan, and reflect the traction you created.',
    };
  }
...
export function buildRefreshSummary({
  ...
  const headline = changedPrimary
    ? 'Your progress changed the strongest move'
    : 'Your progress tightened the plan without forcing a weaker jump';
  const changedSectionText = sectionsUpdated.length
    ? ` It tightened ${sectionsUpdated.slice(0, 3).join(', ').toLowerCase()}.`
    : '';
  const body = changedPrimary
    ? `PivotIQ saw enough new signal to move the main recommendation away from ${previousPrimary.title || 'the old path'} and toward ${nextPrimary.title || 'a stronger path'}.${changedSectionText}`
    : `PivotIQ kept the main recommendation on ${nextPrimary.title || 'the strongest path'} because the new signal still supports the higher-conviction route. The refresh tightened the plan around the progress you logged instead of inventing a riskier jump.${changedSectionText}`;
