...
function jobFilterText(job, fields = ['title', 'department']) {
...
function applySourceFilters(source, jobs) {
  const config = source?.source_config || {};
  const includeTitleKeywords = Array.isArray(config.include_title_keywords) ? config.include_title_keywords : [];
  const excludeTitleKeywords = Array.isArray(config.exclude_title_keywords) ? config.exclude_title_keywords : [];
  const includeTextKeywords = Array.isArray(config.include_text_keywords) ? config.include_text_keywords : [];
  const maxJobs = Number(config.max_jobs || 0);

  let filtered = jobs;

  if (includeTitleKeywords.length) {
    filtered = filtered.filter((job) => matchesAnyKeyword(jobFilterText(job, ['title', 'department']), includeTitleKeywords));
  }

  if (excludeTitleKeywords.length) {
    filtered = filtered.filter((job) => !matchesAnyKeyword(jobFilterText(job, ['title', 'department']), excludeTitleKeywords));
  }

  if (includeTextKeywords.length) {
    filtered = filtered.filter((job) => matchesAnyKeyword(jobFilterText(job, ['title', 'department', 'descriptionText']), includeTextKeywords));
  }

  return maxJobs > 0 ? filtered.slice(0, maxJobs) : filtered;
}

export async function fetchJobsForSource(source) {
  const provider = String(source?.provider || '').toLowerCase();
  let jobs;
  if (provider === 'greenhouse') jobs = await fetchGreenhouseSource(source);
  else if (provider === 'lever') jobs = await fetchLeverSource(source);
  else if (provider === 'smartrecruiters') jobs = await fetchSmartRecruitersSource(source);
  else if (provider === 'remoteok') jobs = await fetchRemoteOkSource(source);
  else if (provider === 'adzuna') jobs = await fetchAdzunaSource(source);
  else throw new Error(`Unsupported provider: ${source?.provider || 'unknown'}`);

  return applySourceFilters(source, jobs);
}
