...
function scoreStartingSkillGap(skill, profile = null) {
  const text = normalizeText([
    skill?.skill_name,
    skill?.category,
    skill?.required_level,
...
function pickStartingSkillGap(skillGaps = [], profile = null) {
  ...
  return [...candidatePool].sort((left, right) => {
    const diff = scoreStartingSkillGap(right, profile) - scoreStartingSkillGap(left, profile);
  ...
  const normalizedReport = {
    ...
    pivots: pivots.map((pivot) => ({
      ...
      learning_path: Array.isArray(pivot.learning_path) && pivot.learning_path.length
        ? buildLearningPath(dedupedPivotSkillGaps, profile)
        : buildLearningPath(dedupedPivotSkillGaps, profile),
        };
      })(),
    })),
    ...
    stay_path: {
      ...
      learning_path: Array.isArray(stayPath?.learning_path) && stayPath.learning_path.length
        ? buildLearningPath(dedupedStayPathSkillGaps, profile)
        : buildLearningPath(dedupedStayPathSkillGaps, profile),
      roadmap: {
