...
function buildAdjacentStayHardSkillResource(skillName = '', roleFamily = 'general', profile = null) {
  ...
  if (/\bfinancial modeling\b|\bscenario\b|\bvariance\b|\bplanning\b/.test(normalized)) {
    return buildStayResource({
      title: 'Financial modeling paths',
      url: 'https://www.datacamp.com/search?q=financial%20modeling',
      provider: 'DataCamp',
  ...
  if (/\bhealth scoring\b|\baccount health\b|\brenewal\b|\blifecycle\b/.test(normalized)) {
  ...
  if (/\battribution\b|\blead lifecycle\b|\bmarketing automation\b|\bcampaign operations\b/.test(normalized)) {
  ...
  if (/\bprocess instrumentation\b|\bexception logic\b|\bworkflow automation\b|\bintake design\b/.test(normalized)) {
...
function buildAdjacentStayHardSkillGap({ profile = null, roleFamily = 'general', primaryTask = '', primarySystem = '', basePivot = null }) {
  ...
  const adjacentSkill = rankedCandidates[0]?.candidate || fallbackByRole[roleFamily];
