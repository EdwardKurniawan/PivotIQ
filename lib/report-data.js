...
function buildLearningPath(skillGaps = []) {
  ...
  const critical = withResources.find((skill) => normalizeText(skill?.gap_priority) === 'critical') || withResources[0];
...
function buildPivotConfidenceState(pivot) {
  ...
  if (openings >= 5 && fitScore >= 35 && missingCount <= 3) {
    return {
      confidence_state: 'market-backed',
      confidence_label: 'Market-backed',
      confidence_reason: `${openings} live opening${openings === 1 ? '' : 's'} matched this path, and your current background already overlaps meaningfully with the repeated requirements.`,
    };
  }
  ...
  if (openings >= 1 || fitScore >= 20 || matchScore >= 78) {
    return {
      ...
      confidence_label: openings ? 'Early market support' : 'Strategy-led, validate with postings',
      confidence_reason: openings
        ? `${openings} live opening${openings === 1 ? '' : 's'} support the direction, but you still need clearer proof before this becomes a high-confidence move.`
        : 'The title is directionally strong for your background, but the current market signal is still too thin to treat it as fully validated.',
    };
  }
  ...
  return {
    ...
    confidence_reason: modelOnlyCount >= 3
      ? 'Several of the remaining gaps still come mostly from model logic rather than repeated live-market demand, so this should be treated as a hypothesis, not a bet.'
      : 'This title is adjacent, but there is not enough clean market evidence yet to rely on it as the main move.',
  };
}

function buildStayConfidenceState(summary, stayAndAdvance) {
  const score = Number(summary?.overall_score || 0);
  ...
  if (score < 45) {
    return {
      ...
      confidence_reason: `The role is not under immediate collapse, so the clearest win is to use AI to move toward ${nextTitle} before you spend energy on a harder external transition.`,
    };
  }
  ...
  if (score < 65) {
    return {
      confidence_state: 'current-lane-advantage',
      confidence_label: 'Strong near-term advantage',
      confidence_reason: `You still have enough leverage inside the current role to turn AI into broader scope, better visibility, and a stronger internal promotion case toward ${nextTitle}.`,
    };
  }
  ...
  return {
    ...
    confidence_reason: `Pressure is rising in the role, so staying only works if you quickly turn AI usage into visible scope, proof, and promotion signal while validating outside options in parallel.`,
  };
}
...
function buildStayOperatingSystem(reportData) {
  ...
  return {
    ...
    first_week_win: normalizeArray(stayAndAdvance?.thirty_day_plan?.this_week)[0] || `Redesign one recurring workflow around ${anchorTask.toLowerCase()} and document the before/after effect.`,
    manager_read: stayAndAdvance?.thirty_day_plan?.leadership_narrative || blueprint.narrative,
    weekly_time_budget: '3-5 focused hours',
  };
}
...
function buildStackItemFromPivot(reportData, pivot, slotLabel = 'Primary move') {
  ...
  const confidence = buildPivotConfidenceState(pivot);
  ...
  return {
    ...
    learning_focus: getFirstLearningStep(pivot),
  };
}
...
function buildStackItemFromStay(reportData, stayPath, slotLabel = 'Stay and advance') {
  ...
  const stayAndAdvance = reportData?.stay_and_advance || {};
  const confidence = buildStayConfidenceState(reportData?.summary || {}, stayAndAdvance);
  ...
  return {
    ...
    learning_focus: getFirstLearningStep(stayPath),
  };
}
...
function buildRecommendationStack(reportData) {
  ...
  const primaryPivotConfidence = buildPivotConfidenceState(primaryPivot || {});
  const riskScore = Number(summary?.overall_score || 0);
  const shouldLeadWithStay = Boolean(
    stayItem
    && (
      riskScore < 55
  ...
  const decisionBriefHeadline = primary?.type === 'stay'
    ? `Use AI to get stronger inside ${reportData?.profile?.job_title || 'your role'} before forcing a pivot.`
...
function buildDecisionFromRecommendationStack(reportData, recommendationStack) {
  ...
  if (primary.type === 'stay') {
    return {
      ...(reportData?.decision || {}),
      recommendation_type: 'stay-and-redesign',
      headline: 'Use AI to strengthen your current lane first',
      urgency: Number(reportData?.summary?.overall_score || 0) >= 65 ? 'Build while pressure is rising' : 'Use this window to get ahead',
  ...
  return {
    ...(reportData?.decision || {}),
    recommendation_type: Number(reportData?.summary?.overall_score || 0) >= 70 ? 'active-pivot' : 'hybrid-transition',
    headline: `Build toward ${primary.title} now`,
    urgency: Number(reportData?.summary?.overall_score || 0) >= 70 ? 'Do not wait for perfect certainty' : 'Build while you still have leverage',
...
function getFirstLearningStep(pivot) {
  const skill = normalizeArray(pivot?.skill_gaps).find((item) => item?.resource_title) || normalizeArray(pivot?.skill_gaps)[0] || null;
...
function buildProofAssetBuilder(reportData) {
  ...
  const builder = {
    ...
    objective: first30Days?.proof_asset?.description || `Show how ${topSkill} improves a realistic business problem connected to ${primaryTask}.`,
    ...
    first_action: `Spend 60 minutes outlining the current ${primaryTask} process and marking where ${topSkill} changes the outcome.`,
    share_prompt: `I’m building proof for a move toward ${targetRole}. Does this artifact make the transition feel credible, and what would you expect to see next?`,
  };
...
function buildStayProofAssetBuilder(reportData) {
  ...
  return {
    title,
    target_role: targetRole,
    objective: stayAndAdvance?.thirty_day_plan?.proof_asset?.description || `Show how you redesigned ${primaryTask.toLowerCase()} with AI in a way leadership can connect to promotion-ready leverage.`,
    ...
    first_action: `Spend 60 minutes picking one workflow around ${primaryTask.toLowerCase()} and writing the before/after operating steps on a single page.`,
    share_prompt: `I’ve redesigned ${primaryTask.toLowerCase()} with AI so the team can move faster without lowering quality. What would make this strong enough to justify broader ownership?`,
    internal_version: {
...
function buildAiLeveragePlaybook(reportData) {
  ...
  const tertiaryTask = primaryTasks[2] || 'team coordination';
...
function buildPaidValueSummary(reportData) {
  ...
  return {
    ...
    first_learning_step: primary?.learning_focus || getFirstLearningStep(pivot),
...
function addPaidValueFields(reportData) {
  ...
  const withBuilder = {
    ...
    pivots: normalizeArray(reportData.pivots).map((pivot) => ({
      ...pivot,
      learning_path: buildLearningPath(pivot?.skill_gaps),
    })),
    stay_path: reportData.stay_path && typeof reportData.stay_path === 'object'
      ? {
          ...reportData.stay_path,
          learning_path: buildLearningPath(reportData.stay_path?.skill_gaps),
        }
      : reportData.stay_path,
...
function buildStayAdvanceSkillGaps(profile, stayAndAdvance, basePivot, locale = 'en') {
  ...
  const resources = {
    workflow: {
      resourceTitle: 'OpenAI Academy',
      resourceUrl: 'https://academy.openai.com/',
      resourceProvider: 'OpenAI Academy',
      resourceAccess: 'free',
      resourceType: 'pathway',
      resourceVerified: true,
      resourceVerificationStatus: 'stay-advance-default',
    },
    communication: {
      resourceTitle: 'Google AI Essentials',
      resourceUrl: 'https://www.coursera.org/google-learn/ai-essentials',
      resourceProvider: 'Google / Coursera',
      resourceAccess: 'paid',
      resourceType: 'course',
      resourceVerified: true,
      resourceVerificationStatus: 'stay-advance-default',
...
export function normalizeReportData(reportData, fallbackProfile = null) {
  ...
  const normalizedReport = {
    ...
    pivots: pivots.map((pivot) => ({
      ...
      learning_path: Array.isArray(pivot.learning_path) && pivot.learning_path.length
        ? pivot.learning_path
        : buildLearningPath(pivot.skill_gaps),
    })),
    ...
    stay_path: {
      ...
      learning_path: Array.isArray(stayPath?.learning_path) && stayPath.learning_path.length
        ...
        : buildLearningPath(stayPath?.skill_gaps),
      roadmap: {
  ...
  return addPaidValueFields(applyReportQualityGate(addPaidValueFields(normalizedReport)));
}
