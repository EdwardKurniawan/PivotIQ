...
function buildLearningPath(skillGaps = [], profile = null, track = 'pivot') {
  ...
  const critical = pickStartingSkillGap(withResources, profile, track) || withResources[0];
  ...
  function helperFor(stepLabel, skill) {
    const skillType = skill?.skill_type || inferStaySkillType(skill?.skill_name || '', inferRoleFamilyFromProfile(profile?.job_title || profile?.jobTitle || '', profile?.industry || '', profile?.tasks || [], profile?.clarifiers || null));
    if (stepLabel === 'Start here') {
      if (skillType === 'system_tool') return 'Use this in the actual system where the workflow already lives.';
      if (skillType === 'hard_skill') return 'Use this to build one sharper layer behind a real decision or workflow.';
      if (skillType === 'workflow_design') return 'Use this to redesign one live workflow before you touch deeper governance or tooling layers.';
      if (skillType === 'governance') return 'Use this to design one repeatable review rhythm or operating rule inside your current team.';
      if (skillType === 'decision_communication') return 'Use this to turn messy output into one crisp readout someone can act on.';
      return 'Learn just enough to redesign one real workflow or decision.';
    }
    if (stepLabel === 'Build proof') {
      if (skillType === 'governance') return 'Turn the idea into a reusable review cadence, checklist, or operating standard.';
      if (skillType === 'decision_communication') return 'Create one memo, readout, or narrative artifact another person can skim quickly.';
      return 'Turn that learning into a visible asset someone else can review in minutes.';
    }
    if (skillType === 'governance') return 'Add depth only after the first operating pattern is already being reused.';
    if (skillType === 'system_tool' || skillType === 'hard_skill') return 'Add depth only after the first artifact is already working in real context.';
    return 'Add depth only after the first artifact is already creating signal.';
  }

  return [
    { label: 'Start here', helper: helperFor('Start here', critical), ...critical },
    { label: 'Build proof', helper: helperFor('Build proof', proof), ...proof },
    { label: 'Go deeper', helper: helperFor('Go deeper', deeper), ...deeper },
...
const TOOL_SKILL_GROUPS = [
  { key: 'python', label: 'Python', pattern: /\bpython\b/ },
  { key: 'sql', label: 'SQL', pattern: /\bsql\b|\bstructured query language\b/ },
  { key: 'power-bi', label: 'Power BI', pattern: /\bpower bi\b|\bdax\b/ },
  { key: 'looker', label: 'Looker', pattern: /\blooker\b/ },
  { key: 'tableau', label: 'Tableau', pattern: /\btableau\b/ },
  { key: 'snowflake', label: 'Snowflake', pattern: /\bsnowflake\b/ },
  { key: 'dbt', label: 'dbt', pattern: /\bdbt\b/ },
  { key: 'salesforce', label: 'Salesforce', pattern: /\bsalesforce\b/ },
  { key: 'hubspot', label: 'HubSpot', pattern: /\bhubspot\b/ },
  { key: 'gainsight', label: 'Gainsight', pattern: /\bgainsight\b/ },
  { key: 'marketo', label: 'Marketo', pattern: /\bmarketo\b/ },
  { key: 'workday', label: 'Workday', pattern: /\bworkday\b/ },
  { key: 'netsuite', label: 'NetSuite', pattern: /\bnetsuite\b/ },
  { key: 'sap', label: 'SAP', pattern: /\bsap\b/ },
  { key: 'jira', label: 'Jira', pattern: /\bjira\b/ },
  { key: 'power-automate', label: 'Power Automate', pattern: /\bpower automate\b/ },
  { key: 'zapier', label: 'Zapier', pattern: /\bzapier\b/ },
  { key: 'excel', label: 'Excel', pattern: /\bexcel\b/ },
  { key: 'anaplan', label: 'Anaplan', pattern: /\banaplan\b/ },
  { key: 'adaptive', label: 'Adaptive Planning', pattern: /\badaptive\b/ },
];

function detectToolSkillGroup(value = '') {
  const normalized = normalizeText(value);
  return TOOL_SKILL_GROUPS.find((group) => group.pattern.test(normalized)) || null;
}

function detectToolSkillGroupForGap(skill = {}) {
  const fromSkillName = detectToolSkillGroup(skill?.skill_name || '');
  if (fromSkillName) return fromSkillName;

  const category = normalizeText(skill?.category);
  if (/\bplatform\b|\bstack\b|\bwarehouse\b|\berp\b|\bhris\b|\btechnical stack\b|\bplatform fluency\b|\btechnical analytics\b/.test(category)) {
    return detectToolSkillGroup([
...
function isHardSkillGap(skill = {}) {
  const text = [
    skill?.skill_name,
    skill?.category,
    skill?.required_level,
  ].join(' ');
  return Boolean(detectToolSkillGroupForGap(skill)) || /\btechnical\b|\bplatform\b|\bstack\b|\bwarehouse\b|\bpipeline\b|\bdata modeling\b|\banalytics engineering\b|\berp\b|\bhris\b/i.test(text);
}

function skillPriorityScore(priority = '') {
...
function chooseLongerText(primary = '', secondary = '') {
  return String(primary || '').length >= String(secondary || '').length ? primary : secondary;
}

function scoreSkillGapQuality(skill = {}) {
  const toolGroup = detectToolSkillGroupForGap(skill);

  return (
    skillPriorityScore(skill?.gap_priority) * 10 +
    (skill?.market_backed ? 8 : 0) +
    (skill?.tool_backed ? 7 : 0) +
    (isHardSkillGap(skill) ? 5 : 0) +
    (toolGroup ? 4 : 0) +
    (skill?.resource_verified ? 3 : 0) +
    (String(skill?.how_to_close_gap || '').length > 80 ? 2 : 0) +
    (String(skill?.required_level || '').length > 60 ? 1 : 0)
  );
}

function mergeSkillGapEntries(existing = {}, candidate = {}) {
  const existingScore = scoreSkillGapQuality(existing);
  const candidateScore = scoreSkillGapQuality(candidate);
  const winner = candidateScore > existingScore ? candidate : existing;
  const loser = winner === candidate ? existing : candidate;
  const group = detectToolSkillGroupForGap(winner) || detectToolSkillGroupForGap(loser);

  return {
    ...loser,
    ...winner,
    skill_name: group?.label || winner?.skill_name || loser?.skill_name || '',
    current_strength: chooseLongerText(winner?.current_strength, loser?.current_strength),
    required_level: chooseLongerText(winner?.required_level, loser?.required_level),
    why_it_matters: chooseLongerText(winner?.why_it_matters, loser?.why_it_matters),
    evidence_you_already_have: chooseLongerText(winner?.evidence_you_already_have, loser?.evidence_you_already_have),
    how_to_close_gap: chooseLongerText(winner?.how_to_close_gap, loser?.how_to_close_gap),
    resource_title: winner?.resource_title || loser?.resource_title || '',
    resource_url: winner?.resource_url || loser?.resource_url || '',
    resource_provider: winner?.resource_provider || loser?.resource_provider || '',
    resource_access: winner?.resource_access || loser?.resource_access || '',
    resource_price_label: winner?.resource_price_label || loser?.resource_price_label || '',
    resource_type: winner?.resource_type || loser?.resource_type || 'course',
    resource_level: winner?.resource_level || loser?.resource_level || '',
    resource_duration_label: winner?.resource_duration_label || loser?.resource_duration_label || '',
    resource_verified: Boolean(winner?.resource_verified || loser?.resource_verified),
    resource_verification_status: winner?.resource_verification_status || loser?.resource_verification_status || '',
    market_backed: Boolean(winner?.market_backed || loser?.market_backed),
    tool_backed: Boolean(winner?.tool_backed || loser?.tool_backed),
  };
}

function dedupeSkillGaps(skillGaps = []) {
  const deduped = [];
  const indexByKey = new Map();

  normalizeArray(skillGaps).forEach((skill) => {
    const normalizedSkill = skill && typeof skill === 'object' ? { ...skill } : skill;
    if (!normalizedSkill || typeof normalizedSkill !== 'object') return;
    const explicitToolGroup = detectToolSkillGroupForGap(normalizedSkill);
    const key = explicitToolGroup
      ? `tool:${explicitToolGroup.key}`
      : normalizeText(normalizedSkill?.skill_name);
    ...
    if (!indexByKey.has(key)) {
      deduped.push({
        ...normalizedSkill,
        skill_name: explicitToolGroup?.label || normalizedSkill.skill_name || '',
      });
    ...
    deduped[index] = mergeSkillGapEntries(deduped[index], normalizedSkill);
  });

  return deduped;
}

function normalizePivotTitleKey(title = '') {
  return normalizeText(String(title || '').replace(/\b(ai|artificial intelligence)\b/gi, '').trim());
}

function dedupePivotsByTitle(pivots = []) {
  const deduped = [];
  const seen = new Set();

  normalizeArray(pivots).forEach((pivot, index) => {
    const key = normalizePivotTitleKey(pivot?.title || pivot?.id || `pivot-${index}`);
...
function buildSystemSpecificStayResource(systemName, fallbackResource) {
  const normalized = normalizeText(systemName);
  const group = detectToolSkillGroup(systemName);

  if (!group) return fallbackResource;

  const exactResources = {
    python: buildStayResource({
      title: 'Python for Data Science and Machine Learning Bootcamp',
      url: 'https://www.udemy.com/course/python-for-data-science-and-machine-learning-bootcamp/',
      provider: 'Udemy',
      access: 'paid',
    }),
    sql: buildStayResource({
      title: 'Learn Structured Query Language (SQL)',
      url: 'https://www.edx.org/learn/sql',
      provider: 'edX',
      access: 'paid',
      type: 'pathway',
    }),
    'power-bi': buildStayResource({
      title: 'Power BI and DAX courses',
      url: 'https://www.udemy.com/courses/search/?q=power%20bi%20dax',
      provider: 'Udemy',
      access: 'paid',
    }),
    looker: buildStayResource({
    ...
    tableau: buildStayResource({
      title: 'Tableau reporting courses',
      url: 'https://www.udemy.com/courses/search/?q=tableau%20reporting',
      provider: 'Udemy',
      access: 'paid',
    }),
    snowflake: buildStayResource({
      title: 'Snowflake Learning Tracks',
      url: 'https://learn.snowflake.com/en/',
      provider: 'Snowflake',
      access: 'free and paid',
      type: 'pathway',
    }),
    dbt: buildStayResource({
      title: 'dbt analytics engineering courses',
      url: 'https://www.udemy.com/courses/search/?q=dbt%20analytics%20engineering',
      provider: 'Udemy',
      access: 'paid',
    }),
    salesforce: buildStayResource({
      title: 'Salesforce admin courses',
      url: 'https://www.udemy.com/courses/search/?q=salesforce%20admin',
      provider: 'Udemy',
      access: 'paid',
    }),
    hubspot: buildStayResource({
      title: 'HubSpot Academy: CRM, marketing, sales, and customer education',
      url: 'https://academy.hubspot.com/',
      provider: 'HubSpot Academy',
      access: 'free',
      type: 'pathway',
    }),
    gainsight: buildStayResource({
      title: 'Gainsight customer operations courses',
      url: 'https://www.udemy.com/courses/search/?q=gainsight%20customer%20success',
      provider: 'Udemy',
      access: 'paid',
    }),
    marketo: buildStayResource({
      title: 'Marketo marketing automation courses',
      url: 'https://www.udemy.com/courses/search/?q=marketo%20marketing%20automation',
      provider: 'Udemy',
      access: 'paid',
    }),
    workday: buildStayResource({
      title: 'Workday reporting and HRIS courses',
      url: 'https://www.udemy.com/courses/search/?q=workday%20reporting%20hris',
      provider: 'Udemy',
      access: 'paid',
    }),
    netsuite: buildStayResource({
      title: 'NetSuite reporting courses',
      url: 'https://www.udemy.com/courses/search/?q=netsuite%20reporting',
      provider: 'Udemy',
      access: 'paid',
    }),
    sap: buildStayResource({
      title: 'SAP reporting and analytics courses',
      url: 'https://www.udemy.com/courses/search/?q=sap%20reporting%20analytics',
      provider: 'Udemy',
      access: 'paid',
    }),
    jira: buildStayResource({
      title: 'Jira and project operations courses',
      url: 'https://www.udemy.com/courses/search/?q=jira%20project%20operations',
      provider: 'Udemy',
      access: 'paid',
    }),
    'power-automate': buildStayResource({
    ...
    zapier: buildStayResource({
      title: 'Learn Zapier in 14 days',
      url: 'https://zapier.com/l/learn-14-days',
      provider: 'Zapier',
      access: 'free',
      type: 'pathway',
    }),
    excel: buildStayResource({
      title: 'Financial modeling paths',
      url: 'https://www.datacamp.com/search?q=financial%20modeling',
      provider: 'DataCamp',
      access: 'paid',
      type: 'pathway',
    }),
    anaplan: buildStayResource({
      title: 'FP&A learning paths',
      url: 'https://www.coursera.org/search?query=fp%26a%20financial%20planning%20analysis',
      provider: 'Coursera',
      access: 'paid',
      type: 'pathway',
    }),
    adaptive: buildStayResource({
      title: 'FP&A learning paths',
      url: 'https://www.coursera.org/search?query=fp%26a%20financial%20planning%20analysis',
      provider: 'Coursera',
      access: 'paid',
  ...
  return exactResources[group.key] || fallbackResource || buildStayResource({
    title: `${systemName} workflow courses`,
    url: normalized ? `https://www.udemy.com/courses/search/?q=${encodeURIComponent(normalized)}` : 'https://www.udemy.com/',
    provider: 'Udemy',
    access: 'paid',
  });
}

function scoreSystemForRoleFamily(systemName = '', roleFamily = 'general') {
  const normalized = normalizeText(systemName);
  const tests = {
    analytics: [
      [/\bpower bi\b|\blooker\b|\btableau\b|\bsql\b|\bsnowflake\b|\bdbt\b/, 12],
      [/\bexcel\b|\bgoogle sheets\b/, 6],
      [/\bsalesforce\b|\bhubspot\b/, 2],
    ],
    finance: [
      [/\bexcel\b|\banaplan\b|\badaptive\b|\bnetsuite\b|\bsap\b|\berp\b/, 12],
      [/\bpower bi\b|\bsql\b|\bsnowflake\b/, 7],
      [/\bsalesforce\b|\bhubspot\b/, 1],
    ],
    hr: [
      [/\bworkday\b|\bhris\b|\bgreenhouse\b|\blever\b|\bbamboohr\b/, 12],
      [/\bpower bi\b|\bexcel\b/, 6],
      [/\bsalesforce\b|\bhubspot\b/, 1],
    ],
    customer: [
      [/\bsalesforce\b|\bhubspot\b|\bgainsight\b/, 12],
      [/\bpower bi\b|\btableau\b|\bexcel\b/, 5],
    ],
    marketing: [
      [/\bhubspot\b|\bmarketo\b|\bgoogle analytics\b|\bga4\b|\bgoogle ads\b/, 12],
      [/\bsalesforce\b/, 8],
      [/\bpower bi\b|\btableau\b/, 5],
    ],
    operations: [
      [/\bjira\b|\bpower automate\b|\bzapier\b|\basana\b|\bmonday\b/, 12],
      [/\bpower bi\b|\bexcel\b/, 6],
    ],
    admin: [
      [/\bcalendar\b|\bnotion\b|\bjira\b|\bpower automate\b/, 12],
      [/\bexcel\b/, 5],
    ],
    general: [[/.+/, 1]],
  };

  const familyRules = tests[roleFamily] || tests.general;
  for (const [pattern, score] of familyRules) {
...
function getSortedRoleSystems(profile = null, roleFamily = 'general') {
  return [...getCoreSystems(profile)].sort((left, right) =>
    scoreSystemForRoleFamily(right, roleFamily) - scoreSystemForRoleFamily(left, roleFamily)
  );
}
...
function hasConcreteSkillSignal(skill) {
  const text = normalizeText([
    skill?.skill_name,
    skill?.category,
    skill?.required_level,
  ...
  return /\b(sql|python|snowflake|bigquery|dbt|etl|elt|warehouse|tableau|looker|power bi|data model|data modeling|database architecture|salesforce|hubspot|gainsight|marketo|ga4|google analytics|workday|hris|erp|sap|netsuite|excel|financial model|forecast model|scenario model|crm|dashboard|kpi|api|integration|power automate|zapier|airtable|lookml)\b/.test(text);
}

function isGenericLearningDetour(skill) {
  const text = normalizeText([
    skill?.skill_name,
    skill?.category,
    skill?.resource_title,
  ...
  if (skill?.market_backed || skill?.tool_backed || hasConcreteSkillSignal(skill)) return false;

  return /\b(business process management|process design|business strategy|digital transformation|prompt design|prompt engineering|ai literacy|google ai essentials|openai academy|anthropic academy|ai for everyone|intro to ai|introduction to ai|fundamentals)\b/.test(text);
}

function scoreStartingSkillGap(skill, profile = null, track = 'pivot') {
  const text = normalizeText([
    skill?.skill_name,
    skill?.category,
    skill?.required_level,
  ...
  const technicalDemand = estimateTechnicalDemand(text);
  let score = 0;

  if (normalizeText(skill?.gap_priority) === 'critical') score += 20;
  else if (normalizeText(skill?.gap_priority) === 'medium') score += 8;

  if (skill?.market_backed) score += 28;
  if (skill?.tool_backed) score += 18;
  if (hasConcreteSkillSignal(skill)) score += 16;
  if (isAdvancedAiWorkflowSkill(skill)) score += hasAdvancedAiMaturity(profile) ? 10 : 6;
  if (!isBeginnerAiSkill(skill)) score += 3;

  const skillType = skill?.skill_type || inferStaySkillType(
    skill?.skill_name || '',
    inferRoleFamilyFromProfile(
  ...
  if (track === 'stay' && skillType === 'workflow_design') score += 14;

  if (isGenericLearningDetour(skill)) score -= 24;
  if (/business process management specialization/.test(text)) score -= 18;
  if (isBeginnerAiSkill(skill) && hasAdvancedAiMaturity(profile)) score -= 10;
  if (technicalDemand > technicalRank + 1) score -= 14;

  return score;
}

function pickStartingSkillGap(skillGaps = [], profile = null, track = 'pivot') {
  const withResources = normalizeArray(skillGaps).filter((skill) => skill?.resource_title && skill?.resource_url);
  if (!withResources.length) return normalizeArray(skillGaps)[0] || null;

  const declaredCapability = getTechnicalCapability(profile);
  const technicalRank = technicalCapabilityRank(declaredCapability);
  const accessiblePool = declaredCapability
    ? withResources.filter((skill) =>
    ...
    : withResources;
  const candidatePool = accessiblePool.some((skill) => !isGenericLearningDetour(skill))
    ? accessiblePool
    : withResources;
  const ranked = [...candidatePool].sort((left, right) => {
    const diff = scoreStartingSkillGap(right, profile, track) - scoreStartingSkillGap(left, profile, track);
    if (diff !== 0) return diff;
    return String(left?.skill_name || '').localeCompare(String(right?.skill_name || ''));
  });

  const best = ranked[0] || candidatePool[0];
  if (track !== 'stay') return best;

  const workflowCandidate = ranked.find((skill) => {
    const skillType = skill?.skill_type || inferStaySkillType(
      skill?.skill_name || '',
      inferRoleFamilyFromProfile(
        profile?.job_title || profile?.jobTitle || '',
    ...
    return skillType === 'workflow_design' && !isGenericLearningDetour(skill);
  });

  if (!workflowCandidate) return best;

  const workflowScore = scoreStartingSkillGap(workflowCandidate, profile, track);
  const bestScore = scoreStartingSkillGap(best, profile, track);
  if (workflowScore >= bestScore - 8) return workflowCandidate;
...
function buildStaySearchResource({
  title,
  provider = 'Coursera',
  query = '',
  access = 'paid',
  ...
  return buildStayResource({
    title,
    url: `${baseUrlByProvider[provider] || baseUrlByProvider.Coursera}${encodeURIComponent(query)}`,
    provider,
    access,
    type,
    verified,
...
function inferStaySkillType(skillName = '', roleFamily = 'general') {
  const normalized = normalizeText(skillName);
  if (!normalized) return 'general';
  if (detectToolSkillGroup(skillName)) return 'system_tool';
  if (/\bnarrative\b|\bstory\b|\breadout\b|\bdecision communication\b|\bbriefing\b/.test(normalized)) return 'decision_communication';
  if (/\bworkflow design\b|\breview design\b|\bexperiment design\b|\bscenario review\b/.test(normalized)) return 'workflow_design';
  if (/\bplanning model governance\b/.test(normalized)) return 'hard_skill';
  if (/\bgovernance\b|\bcadence\b|\boperating standard\b|\boperating system\b|\bplaybook\b|\bhandoff\b|\breview standards\b|\bdecision cadence\b/.test(normalized)) return 'governance';
  if (/\bsql\b|\bpython\b|\bdata modeling\b|\bfinancial modeling\b|\bpeople analytics\b|\bhealth scoring\b|\battribution\b|\blifecycle logic\b|\bexception routing\b|\bmeeting system automation\b|\bplanning model governance\b/.test(normalized)) {
    return 'hard_skill';
  }
  if (['marketing', 'customer', 'operations', 'admin', 'hr', 'finance', 'analytics'].includes(roleFamily) && /\blogic\b|\bscoring\b|\banalytics\b|\bmodel\b/.test(normalized)) {
    return 'hard_skill';
  }
  return 'general';
}

function isSeniorStayProfile(profile = null) {
  const yearsBand = getYearsExperienceBand(profile);
  const currentTitle = profile?.job_title || profile?.jobTitle || '';
  return yearsBand === '6_10' || yearsBand === '11_plus' || titleSeniorityLevel(currentTitle) >= 1;
}

function buildAdvancedStayResource(roleFamily = 'general', skillType = 'general', profile = null) {
  const senior = isSeniorStayProfile(profile);
  const advanced = {
    analytics: {
      workflow_design: buildStaySearchResource({
        title: 'Analytics workflow design paths',
        provider: 'DataCamp',
        query: 'analytics workflow design KPI review',
      }),
      hard_skill: buildStaySearchResource({
        title: 'Data modeling and BI logic paths',
        provider: 'DataCamp',
        query: 'data modeling semantic layer BI logic',
      }),
      governance: buildStaySearchResource({
        title: 'Analytics Review and Metric Governance',
        provider: 'DataCamp',
        query: 'data governance analytics storytelling',
      }),
      decision_communication: buildStaySearchResource({
        title: 'Analytics storytelling paths',
        provider: 'DataCamp',
        query: 'analytics storytelling',
      }),
    },
    finance: {
      workflow_design: buildStaySearchResource({
        title: 'Finance Workflow Design',
        query: 'finance workflow design scenario review',
      }),
      hard_skill: buildStaySearchResource({
        title: 'Finance Modeling and Planning Logic',
        query: 'financial modeling planning logic scenario analysis',
      }),
      governance: buildStaySearchResource({
        title: 'Finance Planning Operating Cadence',
        query: 'finance planning operations cadence',
      }),
      decision_communication: buildStaySearchResource({
        title: 'Finance decision support paths',
        query: 'finance decision support planning narrative',
      }),
    },
    hr: {
      workflow_design: buildStaySearchResource({
        title: 'Manager Enablement Workflow Design',
        query: 'manager enablement workflow design people operations',
      }),
      hard_skill: buildStaySearchResource({
        title: 'People Analytics and Workforce Planning',
        provider: 'Udemy',
        query: 'people analytics workforce planning',
      }),
      governance: buildStaySearchResource({
        title: 'People Systems and Manager Operations',
        query: 'people operations systems manager enablement',
      }),
      decision_communication: buildStaySearchResource({
        title: 'People analytics and workforce planning paths',
        provider: 'Udemy',
        query: 'people analytics workforce planning',
      }),
    },
    customer: {
      workflow_design: buildStaySearchResource({
        title: 'Customer Workflow Design',
        provider: 'Udemy',
        query: 'customer success workflow design renewal review',
      }),
      hard_skill: buildStaySearchResource({
        title: 'Customer Health and Renewal Analysis',
        provider: 'Udemy',
        query: 'customer health scoring renewal analysis',
      }),
      governance: buildStaySearchResource({
        title: 'Customer Renewal Playbook Systems',
        provider: 'Udemy',
        query: 'customer success operations playbook',
      }),
      decision_communication: buildStaySearchResource({
        title: 'Customer review and renewal strategy paths',
        provider: 'Udemy',
        query: 'customer renewal strategy account review',
      }),
    },
    marketing: {
      workflow_design: buildStaySearchResource({
        title: 'Campaign Workflow and Experimentation',
        query: 'campaign experimentation workflow design',
      }),
      hard_skill: buildStaySearchResource({
        title: 'Marketing Attribution and Lifecycle Analysis',
        provider: 'DataCamp',
        query: 'marketing attribution lifecycle analytics',
      }),
      governance: buildStaySearchResource({
        title: 'Marketing Lifecycle Operating System',
        query: 'marketing operations lifecycle management',
      }),
      decision_communication: buildStaySearchResource({
        title: 'Marketing analytics and growth review paths',
        provider: 'DataCamp',
        query: 'marketing analytics attribution experimentation',
      }),
    },
    operations: {
      workflow_design: buildStaySearchResource({
        title: 'Workflow Design and Process Mapping',
        query: 'workflow design process mapping operations',
      }),
      hard_skill: buildStaySearchResource({
        title: 'Exception Routing and Process Control',
        query: 'exception routing process control operations',
      }),
      governance: buildStaySearchResource({
        title: 'Process Handoff Governance Design',
        query: 'process governance handoff design',
      }),
      decision_communication: buildStaySearchResource({
        title: 'Operating review and business analysis paths',
        query: 'business analysis operating review',
      }),
    },
    admin: {
      workflow_design: buildStaySearchResource({
        title: 'Executive Workflow Design',
        query: 'executive workflow design chief of staff',
      }),
      hard_skill: buildStaySearchResource({
        title: 'Executive Operating Systems and Meeting Automation',
        query: 'executive operations meeting automation',
      }),
      governance: buildStaySearchResource({
        title: 'Executive Operations and Decision Cadence',
        query: 'chief of staff executive operations',
      }),
      decision_communication: buildStaySearchResource({
        title: 'Executive communication and briefing paths',
        provider: "O'Reilly",
        query: 'executive communication briefing',
      }),
    },
  };

  const roleSet = advanced[roleFamily] || {};
  if (skillType === 'workflow_design' && roleSet.workflow_design) return roleSet.workflow_design;
  if (skillType === 'hard_skill' && roleSet.hard_skill) return roleSet.hard_skill;
  if (skillType === 'governance' && roleSet.governance) return roleSet.governance;
  if (skillType === 'decision_communication' && roleSet.decision_communication) return roleSet.decision_communication;
  if (senior && roleSet.workflow_design) return roleSet.workflow_design;
  if (senior && roleSet.governance) return roleSet.governance;
  return null;
}
...
function buildStayLearningResources(roleFamily, profile = null) {
  ...
  const hasFinanceSystems = coreSystemsInclude(profile, /\bexcel\b|\bgoogle sheets\b|\bsheets\b|\banaplan\b|\badaptive\b|\bplanning\b|\berp\b/);
  const advancedWorkflowResource = buildAdvancedStayResource(roleFamily, 'workflow_design', profile);
  const advancedLeadershipResource = buildAdvancedStayResource(roleFamily, 'governance', profile);
  const advancedCommunicationResource = buildAdvancedStayResource(roleFamily, 'decision_communication', profile);

  const defaults = {
    workflow: advancedWorkflowResource || buildStayResource({
    ...
    communication: advancedCommunicationResource || buildStayResource({
    ...
    leadership: advancedLeadershipResource || buildStayResource({
  ...
  if (roleFamily === 'finance') {
    return {
      workflow: advancedWorkflowResource || buildStayResource({
        title: hasFinanceSystems ? 'FP&A learning paths' : 'Financial modeling paths',
        url: hasFinanceSystems ? 'https://www.coursera.org/search?query=fp%26a%20financial%20planning%20analysis' : 'https://www.datacamp.com/search?q=financial%20modeling',
        provider: hasFinanceSystems ? 'Coursera' : 'DataCamp',
        access: 'paid',
        type: 'pathway',
      }),
      communication: advancedCommunicationResource || buildStayResource({
      ...
      leadership: advancedLeadershipResource || buildStayResource({
        title: hasPowerAutomate ? 'Build and optimize cloud flows in Power Automate' : 'Finance automation learning paths',
        url: hasPowerAutomate ? 'https://learn.microsoft.com/en-us/training/paths/build-optimize-cloud-flows-power-automate/' : 'https://www.coursera.org/search?query=finance%20automation',
  ...
  if (roleFamily === 'customer') {
    return {
      ...
      communication: advancedCommunicationResource || buildStayResource({
        title: hasHubspot ? 'HubSpot Academy: CRM, marketing, sales, and customer education' : hasSalesforce ? 'Trailhead: CRM and Salesforce learning' : 'Google AI Essentials',
        url: hasHubspot ? 'https://academy.hubspot.com/' : hasSalesforce ? 'https://trailhead.salesforce.com/' : 'https://www.coursera.org/google-learn/ai-essentials',
        provider: hasHubspot ? 'HubSpot Academy' : hasSalesforce ? 'Salesforce Trailhead' : 'Google / Coursera',
        access: hasHubspot || hasSalesforce ? 'free' : 'paid',
        type: hasHubspot || hasSalesforce ? 'pathway' : 'course',
      }),
      leadership: advancedLeadershipResource || buildStayResource({
        title: hasSalesforce ? 'Improve Business with Salesforce Flow Automation' : hasHubspot ? 'Service Hub Software Certification Course' : 'Customer success operations courses',
        url: hasSalesforce ? 'https://trailhead.salesforce.com/content/learn/trails/distribute-and-implement-flows' : hasHubspot ? 'https://academy.hubspot.com/courses/service-hub-software' : 'https://www.udemy.com/courses/search/?q=customer%20success%20operations',
        provider: hasSalesforce ? 'Salesforce Trailhead' : hasHubspot ? 'HubSpot Academy' : 'Udemy',
        access: hasSalesforce || hasHubspot ? 'free' : 'paid',
        type: hasSalesforce ? 'course' : hasHubspot ? 'course' : 'course',
      }),
    };
  }
  ...
  if (roleFamily === 'operations' || roleFamily === 'product') {
    return {
      workflow: advancedWorkflowResource || buildStayResource({
        title: hasPowerAutomate ? 'Build and optimize cloud flows in Power Automate' : hasJira ? 'Atlassian University: Jira and workflow management' : 'Learn Zapier in 14 days',
        url: hasPowerAutomate ? 'https://learn.microsoft.com/en-us/training/paths/build-optimize-cloud-flows-power-automate/' : hasJira ? 'https://university.atlassian.com/' : 'https://zapier.com/l/learn-14-days',
        provider: hasPowerAutomate ? 'Microsoft Learn' : hasJira ? 'Atlassian University' : 'Zapier',
        access: 'free',
        type: 'pathway',
      }),
      communication: advancedCommunicationResource || buildStayResource({
        title: hasJira ? 'Atlassian University: Jira and workflow management' : 'Business analysis learning paths',
        url: hasJira ? 'https://university.atlassian.com/' : 'https://www.coursera.org/search?query=business%20analysis',
        provider: hasJira ? 'Atlassian University' : 'Coursera',
        access: hasJira ? 'free' : 'paid',
        type: hasJira ? 'pathway' : 'pathway',
      }),
      leadership: advancedLeadershipResource || buildStayResource({
        title: hasPowerAutomate ? 'Build and optimize cloud flows in Power Automate' : 'Change management learning paths',
        url: hasPowerAutomate ? 'https://learn.microsoft.com/en-us/training/paths/build-optimize-cloud-flows-power-automate/' : 'https://www.coursera.org/search?query=change%20management',
        provider: hasPowerAutomate ? 'Microsoft Learn' : 'Coursera',
        access: hasPowerAutomate ? 'free' : 'paid',
        type: hasPowerAutomate ? 'pathway' : 'pathway',
      }),
...
function buildEnglishStaySkillBlueprints(roleFamily, profile = null) {
  ...
  const roleSpecific = {
    marketing: {
      ...
      leadership: {
        name: 'Growth review operating system',
    ...
    analytics: {
      ...
      leadership: {
        name: 'Analysis review standards',
    ...
    hr: {
      ...
      leadership: {
        name: 'Manager support operating standard',
    ...
    admin: {
      ...
      leadership: {
        name: 'Executive decision cadence',
    ...
    customer: {
      ...
      leadership: {
        name: 'Renewal playbook governance',
    ...
    operations: {
      ...
      leadership: {
        name: 'Exception and handoff governance',
...
function buildStaySystemSkillGap({ profile = null, roleFamily = 'general', primaryTask = '', fallbackResource = null }) {
  const systems = getSortedRoleSystems(profile, roleFamily);
  const primarySystem = systems[0];
  if (!primarySystem) return null;

  const systemLabel = detectToolSkillGroup(primarySystem)?.label || primarySystem;
  const normalizedSystem = normalizeText(primarySystem);
  const resource = buildSystemSpecificStayResource(primarySystem, fallbackResource);

  const blueprints = [
    {
      match: /\bpower bi\b|\blooker\b|\btableau\b/,
      name: `${systemLabel} review and dashboard governance`,
      required: `You can use ${systemLabel} to make ${primaryTask.toLowerCase()} faster while tightening dashboard trust, semantic logic, and the review checkpoint before anything gets shared.`,
      why: `Teams do not just need more dashboards. They need someone who can make ${systemLabel} output decision-ready and trustworthy.`,
      close: `Use ${systemLabel} on one high-visibility review pack, document the review logic, and show where AI speeds the first pass without weakening the final call.`,
    },
    {
      match: /\bsql\b|\bsnowflake\b|\bdbt\b/,
      name: `${systemLabel} pipeline and data-quality fluency`,
      required: `You can use ${systemLabel} to trace data issues, tighten transformations, and explain why the final output is safe to use.`,
      why: `Hard-skill leverage here comes from controlling data quality and review logic, not just producing another extract.`,
      close: `Rebuild one recurring transformation, QA, or warehouse review in ${systemLabel} and show how the new process reduces rework or bad signals.`,
    },
    {
      match: /\bsalesforce\b|\bhubspot\b|\bgainsight\b|\bmarketo\b/,
      name: `${systemLabel} workflow orchestration`,
      required: `You can use ${systemLabel} to redesign lifecycle, account, or campaign workflows with cleaner automation and clearer human checkpoints.`,
      why: `Owning the system logic is what makes you more valuable than simply working cases or pushing updates through the tool.`,
      close: `Use ${systemLabel} to redesign one recurring customer or growth workflow and show where AI now shortens prep work without weakening judgment.`,
    },
    {
      match: /\bworkday\b|\bhris\b/,
      name: `${systemLabel} people-systems fluency`,
      required: `You can use ${systemLabel} to improve reporting, intake, and policy workflows while keeping exception handling under human control.`,
      why: `People teams become safer when someone can turn HR systems into reliable operating support instead of one-off manual work.`,
      close: `Map one manager-support or reporting workflow inside ${systemLabel}, then show how AI and system rules reduce repeat questions or delays.`,
    },
    {
      match: /\bexcel\b|\banaplan\b|\badaptive\b|\bnetsuite\b|\bsap\b/,
      name: `${systemLabel} planning and review governance`,
      required: `You can use ${systemLabel} to tighten model logic, review rhythm, and decision-readiness around recurring finance work.`,
      why: `Finance leverage comes from controlling how planning moves, not just from updating the file faster.`,
      close: `Rebuild one planning or review cycle in ${systemLabel}, document the control points, and show how AI now improves speed without weakening finance judgment.`,
    },
    {
      match: /\bjira\b|\bpower automate\b|\bzapier\b/,
      name: `${systemLabel} workflow automation controls`,
      required: `You can use ${systemLabel} to standardize routing, approvals, and exception handling around one recurring workflow.`,
      why: `Operations leverage shows up when the system runs cleaner because you designed the controls, not because you manually carried the work.`,
      close: `Use ${systemLabel} to rebuild one intake, coordination, or approval workflow and show the before/after operating change.`,
    },
  ];

  const blueprint = blueprints.find((item) => item.match.test(normalizedSystem)) || {
    name: `${systemLabel} operating fluency`,
    required: `You can use ${systemLabel} to redesign a recurring workflow with clearer review logic, stronger inputs, and better decision support.`,
    why: `Hard-skill leverage shows up when you turn ${systemLabel} into part of a safer, faster operating system instead of just another tool you touch.`,
    close: `Use ${systemLabel} on one visible workflow, document the new operating pattern, and show the business effect clearly.`,
  };

  return buildSkillGap({
    skillName: blueprint.name,
    skillType: 'system_tool',
    category: 'technical stack',
    currentStrength: `You already work inside ${systemLabel} and understand where ${primaryTask.toLowerCase()} slows down or gets noisy.`,
    requiredLevel: blueprint.required,
    gapPriority: 'critical',
    whyItMatters: blueprint.why,
    evidence: `You already have live context for ${primaryTask.toLowerCase()}, so you can turn ${systemLabel} into visible proof instead of theoretical learning.`,
    howToClose: blueprint.close,
    ...resource,
  });
}

function scoreAdjacentStayHardSkillCandidate({
  candidate = '',
  roleFamily = 'general',
  primarySystem = '',
  technicalCapability = '',
  ...
  if (!normalized) return 0;
  if (/\bprompt\b|\bbusiness process management\b|\bai workflow\b|\bworkflow design\b|\bdecision communication\b|\badoption leadership\b|\bai qa\b|\bautomation mapping\b/.test(normalized)) {
    return 0;
  }

  const primaryGroup = detectToolSkillGroup(primarySystem)?.key || '';
  const candidateGroup = detectToolSkillGroup(candidate)?.key || '';
  if (candidateGroup && primaryGroup && candidateGroup === primaryGroup) return 0;
  if (!primarySystem && candidateGroup && ['customer', 'marketing', 'hr', 'operations', 'admin'].includes(roleFamily)) {
    const normalizedGroupLabel = normalizeText(detectToolSkillGroup(candidate)?.label || '');
    if (normalized === normalizedGroupLabel || normalized === candidateGroup.replace(/-/g, ' ')) {
      return 0;
    }
  }

  const capability = technicalCapabilityRank(technicalCapability);
  let score = isHardSkillGap({ skill_name: candidate, category: 'technical stack' }) ? 4 : 0;

  const familyPatterns = {
    analytics: [
      [/\bsql\b|\bdata modeling\b|\bsemantic\b|\bdatabase architecture\b/, 14],
      [/\bsnowflake\b|\bdbt\b|\betl\b|\bpipeline\b/, capability >= 2 ? 12 : 8],
      [/\bpython\b/, capability >= 2 ? 11 : 6],
      [/\bpower bi\b|\blooker\b|\btableau\b/, 10],
    ],
    finance: [
      [/\bfinancial modeling\b|\bscenario\b|\bvariance\b|\bplanning\b/, 14],
      [/\berp\b|\bnetsuite\b|\bsap\b|\banaplan\b|\badaptive\b/, 12],
      [/\bpower bi\b|\bsql\b|\bsnowflake\b/, capability >= 2 ? 10 : 7],
    ],
    customer: [
      [/\bgainsight\b|\bsalesforce\b|\bhubspot\b/, 13],
      [/\bhealth scoring\b|\baccount health\b|\brenewal\b|\blifecycle\b/, 12],
      [/\breporting\b|\bdashboard\b/, 8],
    ],
    marketing: [
      [/\bhubspot\b|\bmarketo\b|\bga4\b|\bgoogle analytics\b/, 13],
      [/\battribution\b|\blead lifecycle\b|\bcampaign operations\b|\bmarketing automation\b/, 12],
      [/\breporting\b|\bdashboard\b/, 8],
    ],
    hr: [
      [/\bworkday\b|\bhris\b/, 13],
      [/\bpeople analytics\b|\bworkforce analytics\b|\breporting\b/, 12],
      [/\bworkflow automation\b|\bpower automate\b/, 8],
    ],
    operations: [
      [/\bpower automate\b|\bzapier\b|\bjira\b/, 13],
      [/\bprocess instrumentation\b|\bexception logic\b|\bworkflow automation\b|\bintake design\b/, 12],
      [/\breporting\b|\bdashboard\b/, 7],
    ],
    admin: [
      [/\bpower automate\b|\bzapier\b|\bnotion\b/, 12],
      [/\bbriefing system\b|\bmeeting system\b|\bfollow-up automation\b/, 11],
    ],
    general: [[/\bworkflow automation\b|\breporting\b|\banalytics\b/, 6]],
  };

  for (const [pattern, value] of familyPatterns[roleFamily] || familyPatterns.general) {
...
function buildAdjacentStayHardSkillResource(skillName = '', roleFamily = 'general', profile = null) {
  const toolResource = buildSystemSpecificStayResource(skillName, null);
  if (toolResource) return toolResource;

  const normalized = normalizeText(skillName);
  const skillType = inferStaySkillType(skillName, roleFamily);
  const advancedResource = buildAdvancedStayResource(roleFamily, skillType, profile);
  if (advancedResource && skillType === 'governance') return advancedResource;

  if (/\bdatabase architecture\b|\bdata modeling\b|\bsemantic\b|\betl\b|\bpipeline\b|\banalytics engineering\b/.test(normalized)) {
    return buildStayResource({
      title: 'Data engineering learning paths',
      url: 'https://www.pluralsight.com/paths/data-engineering',
  ...
  if (/\bfinancial modeling\b|\bscenario\b|\bvariance\b|\bplanning\b|\bmodel governance\b/.test(normalized)) {
    if (/\bplanning review cadence\b/.test(normalized)) {
      const governanceResource = buildAdvancedStayResource(roleFamily, 'governance', profile);
      if (governanceResource) return governanceResource;
    }
    if (/\bmodel governance\b/.test(normalized)) {
      const hardSkillResource = buildAdvancedStayResource(roleFamily, 'hard_skill', profile);
      if (hardSkillResource) return hardSkillResource;
    }
    if (advancedResource && /\bmodel governance\b|\bplanning review cadence\b/.test(normalized)) {
  ...
  if (/\bpeople analytics\b|\bworkforce analytics\b/.test(normalized)) {
    if (advancedResource) return advancedResource;
    return buildStayResource({
      title: 'Workday reporting and HRIS courses',
      url: 'https://www.udemy.com/courses/search/?q=workday%20reporting%20hris',
      provider: 'Udemy',
  ...
  if (/\bhealth scoring\b|\baccount health\b|\brenewal\b|\bcustomer lifecycle\b/.test(normalized)) {
    if (coreSystemsInclude(profile, /\bgainsight\b/)) {
      return buildSystemSpecificStayResource('Gainsight', null);
    }
    if (coreSystemsInclude(profile, /\bsalesforce\b/)) {
      return buildSystemSpecificStayResource('Salesforce', null);
    }
    if (advancedResource) return advancedResource;
    return buildStayResource({
      title: 'Customer success operations courses',
      url: 'https://www.udemy.com/courses/search/?q=customer%20success%20operations',
      provider: 'Udemy',
      access: 'paid',
    });
  }

  if (/\battribution\b|\blead lifecycle\b|\blifecycle logic\b|\bmarketing automation\b|\bcampaign operations\b/.test(normalized)) {
    if (coreSystemsInclude(profile, /\bmarketo\b/)) {
      return buildSystemSpecificStayResource('Marketo', null);
    }
    if (coreSystemsInclude(profile, /\bhubspot\b/)) {
      return buildSystemSpecificStayResource('HubSpot', null);
    }
    if (advancedResource) return advancedResource;
    return buildStayResource({
      title: 'AI for Marketing Course',
  ...
  if (/\bprocess instrumentation\b|\bexception logic\b|\bexception routing\b|\bworkflow automation\b|\bintake design\b/.test(normalized)) {
    if (coreSystemsInclude(profile, /\bpower automate\b/)) {
      return buildSystemSpecificStayResource('Power Automate', null);
    }
    if (coreSystemsInclude(profile, /\bjira\b/)) {
      return buildSystemSpecificStayResource('Jira', null);
    }
    if (advancedResource) return advancedResource;
    return buildSystemSpecificStayResource('Zapier', null);
  }

  if (skillType === 'decision_communication' && advancedResource) {
    return advancedResource;
  }

  const familyFallbacks = {
    analytics: buildStayResource({
      title: 'Get started with Microsoft data analytics',
      url: 'https://learn.microsoft.com/en-us/training/paths/data-analytics-microsoft/',
      provider: 'Microsoft Learn',
      access: 'free',
    ...
    customer: buildStayResource({
      title: 'Customer success operations courses',
      url: 'https://www.udemy.com/courses/search/?q=customer%20success%20operations',
      provider: 'Udemy',
      access: 'paid',
    }),
    marketing: buildStayResource({
      title: 'AI for Marketing Course',
      url: 'https://academy.hubspot.com/courses/ai-for-marketers',
      provider: 'HubSpot Academy',
      access: 'free',
    }),
    hr: buildStayResource({
      title: 'Workday reporting and HRIS courses',
      url: 'https://www.udemy.com/courses/search/?q=workday%20reporting%20hris',
      provider: 'Udemy',
      access: 'paid',
    }),
    operations: buildStayResource({
      title: 'Learn Zapier in 14 days',
      url: 'https://zapier.com/l/learn-14-days',
      provider: 'Zapier',
      access: 'free',
  ...
  return familyFallbacks[roleFamily] || buildStayResource({
    title: 'OpenAI Academy',
    url: 'https://academy.openai.com/',
    provider: 'OpenAI Academy',
    access: 'free',
...
function buildAdjacentStayHardSkillGap({ profile = null, roleFamily = 'general', primaryTask = '', primarySystem = '', basePivot = null }) {
  const technicalCapability = getTechnicalCapability(profile);
  const primaryGroup = detectToolSkillGroup(primarySystem)?.key || '';
  const candidatePool = [
    ...normalizeArray(basePivot?.skill_gaps).map((item) => item?.skill_name).filter(Boolean),
    ...normalizeArray(basePivot?.live_market_signal?.market_required_skills),
    ...normalizeArray(basePivot?.live_market_signal?.market_tools),
  ];
  ...
  const rankedCandidates = candidatePool
    ...
    .map((candidate) => ({
      candidate,
      score: scoreAdjacentStayHardSkillCandidate({
        candidate,
        roleFamily,
        primarySystem,
        technicalCapability,
  ...
  const fallbackByRole = {
    analytics: technicalCapabilityRank(technicalCapability) >= 2 ? 'Data modeling' : 'SQL',
    finance: 'Planning model governance',
    customer: 'Customer health scoring',
    marketing: 'Attribution and lifecycle logic',
    hr: 'People analytics',
    operations: 'Exception routing logic',
    admin: 'Meeting system automation',
  };

  let adjacentSkill = rankedCandidates[0]?.candidate || fallbackByRole[roleFamily];
  if (!adjacentSkill) return null;

  const primaryWorkflowByRole = {
    analytics: 'Dashboard QA workflow design',
    finance: 'Financial modeling and scenario review',
    customer: 'Renewal risk review design',
    marketing: 'Campaign experiment design',
    hr: 'Manager enablement workflow design',
    operations: 'Workflow automation design',
    admin: 'Executive workflow design',
  };
  const primaryWorkflow = normalizeText(primaryWorkflowByRole[roleFamily] || '');
  const normalizedAdjacent = normalizeText(adjacentSkill);
  if (
    primaryWorkflow &&
    (
      primaryWorkflow.includes(normalizedAdjacent) ||
      normalizedAdjacent.includes(primaryWorkflow) ||
      overlapCount(tokenSet(primaryWorkflow), tokenSet(normalizedAdjacent)) >= 2
    ) &&
    fallbackByRole[roleFamily]
  ) {
    adjacentSkill = fallbackByRole[roleFamily];
  }

  const adjacentGroup = detectToolSkillGroup(adjacentSkill)?.key || '';
  if (adjacentGroup && primaryGroup && adjacentGroup === primaryGroup && !fallbackByRole[roleFamily]) {
    return null;
  }

  const adjacentText = {
    analytics: {
      current: `You already have enough reporting context to see where ${primaryTask.toLowerCase()} still depends on better data structure and cleaner logic.`,
      required: `You can use ${adjacentSkill} to tighten the data layer behind ${primaryTask.toLowerCase()}, not just the final dashboard or readout.`,
      why: 'That is what turns a reporting user into someone who can protect the trustworthiness of the analysis itself.',
      close: `Use ${adjacentSkill} on one recurring analytics workflow and show how it improved trust, speed, or review quality.`,
    },
    finance: {
      current: `You already know where ${primaryTask.toLowerCase()} gets slow or brittle because the underlying model or planning logic is hard to trust quickly.`,
      required: `You can use ${adjacentSkill} to make the planning layer behind ${primaryTask.toLowerCase()} easier to review, challenge, and reuse.`,
      why: 'Finance gets stronger when you control the planning logic and review cadence, not just the final number.',
      close: `Use ${adjacentSkill} on one planning or variance-review workflow and show how it improved decision speed without weakening control.`,
    },
    customer: {
      current: `You already know where ${primaryTask.toLowerCase()} depends on fragmented account signals or inconsistent CRM hygiene.`,
      required: `You can use ${adjacentSkill} to make the customer signal layer cleaner before the team acts on it.`,
      why: 'That is what moves customer work from reactive coordination into trusted commercial judgment.',
      close: `Apply ${adjacentSkill} to one renewal, onboarding, or health-review workflow and show where the signal quality improved.`,
    },
    marketing: {
      current: `You already know where ${primaryTask.toLowerCase()} still depends on messy attribution, brittle automation, or scattered performance context.`,
      required: `You can use ${adjacentSkill} to strengthen the system logic behind campaign decisions, not just the visible output.`,
      why: 'That is how marketing work becomes harder to replace and easier to trust.',
      close: `Use ${adjacentSkill} on one campaign or lifecycle workflow and show how it improved planning or review quality.`,
    },
    hr: {
      current: `You already know where ${primaryTask.toLowerCase()} still depends on scattered reporting, inconsistent people-system inputs, or repeat questions.`,
      required: `You can use ${adjacentSkill} to make the support and insight layer behind ${primaryTask.toLowerCase()} cleaner and more reliable.`,
      why: 'People teams become stronger when the underlying system logic is safer and easier to trust.',
      close: `Apply ${adjacentSkill} to one people-ops or manager-support workflow and show how it reduced friction or improved review quality.`,
    },
    operations: {
      current: `You already know where ${primaryTask.toLowerCase()} still depends on brittle routing, vague exceptions, or weak workflow visibility.`,
      required: `You can use ${adjacentSkill} to make the operating logic behind ${primaryTask.toLowerCase()} clearer and more repeatable.`,
      why: 'Operations leverage compounds when the system can explain itself, not just move work faster.',
      close: `Use ${adjacentSkill} on one intake or coordination workflow and show how it improved control, clarity, or throughput.`,
    },
    admin: {
      current: `You already know where ${primaryTask.toLowerCase()} still depends on scattered context, repeated follow-up, or manual synthesis.`,
      required: `You can use ${adjacentSkill} to make the support layer behind ${primaryTask.toLowerCase()} cleaner and easier to trust.`,
      why: 'That is how support work becomes visible operating leverage instead of invisible catch-up work.',
      close: `Use ${adjacentSkill} on one briefing, meeting, or follow-up workflow and show how it improved clarity or execution speed.`,
    },
    general: {
      current: `You already know where ${primaryTask.toLowerCase()} still depends on weak system logic or inconsistent inputs.`,
      required: `You can use ${adjacentSkill} to strengthen the layer underneath ${primaryTask.toLowerCase()}, not just the surface output.`,
      why: 'That is what turns workflow familiarity into durable leverage.',
      close: `Use ${adjacentSkill} on one recurring workflow and show how it made the work safer, faster, or easier to review.`,
    },
  };

  const copy = adjacentText[roleFamily] || adjacentText.general;
  const resource = buildAdjacentStayHardSkillResource(adjacentSkill, roleFamily, profile);

  return buildSkillGap({
    skillName: adjacentSkill,
    skillType: inferStaySkillType(adjacentSkill, roleFamily),
    category: 'technical depth',
    currentStrength: copy.current,
    requiredLevel: copy.required,
    gapPriority: 'medium',
    whyItMatters: copy.why,
    evidence: `You already have enough live context from ${primaryTask.toLowerCase()} to build proof with ${adjacentSkill} instead of learning it in the abstract.`,
    howToClose: copy.close,
    ...resource,
  });
}
...
function buildAiThisWeekPlan(reportData) {
  ...
  const defaultSystems = (() => {
    if (roleFamily === 'legal') return ['CLM or intake queue', 'request notes', 'review checklist'];
    if (roleFamily === 'procurement') return ['supplier intake', 'vendor comparison sheet', 'sourcing review'];
    if (roleFamily === 'education') return ['LMS or enablement hub', 'content brief', 'learner feedback review'];
    ...
    if (roleFamily === 'product') return ['planning doc', 'research notes', 'decision memo'];
    return ['the system where this workflow already lives'];
  })();
  const operatorFrames = {
    legal: {
      weakening_now: 'Manual intake routing, request summarization, and repetitive first-pass legal packaging are getting cheaper first.',
      compounding_now: 'Review logic, risk judgment, and the workflow standards other teams must trust still compound.',
      human_owned: 'Keep approval logic, exception handling, and legal-risk judgment human-owned.',
      business_result: 'Reduce request-routing friction and improve legal review quality.',
    },
    procurement: {
      weakening_now: 'Vendor-note cleanup, comparison formatting, and repetitive sourcing summaries get cheaper first.',
      compounding_now: 'Supplier judgment, stakeholder trade-offs, and decision-ready sourcing recommendations still compound.',
      human_owned: 'Keep supplier judgment, commercial trade-offs, and final recommendations human-owned.',
      business_result: 'Improve sourcing speed and supplier-decision clarity for stakeholders.',
    },
    education: {
      weakening_now: 'First-pass content drafting, update packaging, and routine enablement assembly get cheaper first.',
      compounding_now: 'Instructional judgment, audience fit, and the learning system teams rely on still compound.',
      human_owned: 'Keep learner context, instructional quality, and final enablement decisions human-owned.',
      business_result: 'Shorten content-update cycles while improving learner readiness and adoption.',
    },
    marketing: {
      weakening_now: 'Templated campaign planning, reporting, and first-pass briefing work thin out first.',
      compounding_now: 'Experiment judgment, channel trade-offs, and growth-readout quality still compound.',
      human_owned: 'Keep targeting decisions, brand/context judgment, and final go/no-go calls human-owned.',
      business_result: 'Increase experiment velocity without lowering campaign quality.',
    },
    analytics: {
      weakening_now: 'Raw dashboard pulls, recurring KPI commentary, and first-pass trend summaries get cheaper first.',
      compounding_now: 'Metric trust, anomaly judgment, and business recommendations leaders can act on still compound.',
      human_owned: 'Keep QA, metric interpretation, and the final business recommendation human-owned.',
      business_result: 'Reduce dashboard-to-decision time while improving trust in the numbers.',
    },
    finance: {
      weakening_now: 'Manual variance packaging, recurring scenario prep, and first-pass finance commentary get cheaper first.',
      compounding_now: 'Assumption judgment, scenario framing, and leadership recommendation quality still compound.',
      human_owned: 'Keep model logic, assumption review, and trade-off calls human-owned.',
      business_result: 'Cut planning-cycle drag while improving decision-readiness for leaders.',
    },
    hr: {
      weakening_now: 'Routine policy drafting, repeated manager responses, and standard people-process packaging get cheaper first.',
      compounding_now: 'Sensitive judgment, exception handling, and people-system design still compound.',
      human_owned: 'Keep sensitive case handling, context judgment, and policy exceptions human-owned.',
      business_result: 'Improve support consistency while reducing repeated HR escalations.',
    },
    customer: {
      weakening_now: 'Routine account recaps, note cleanup, and repetitive prep work get cheaper first.',
      compounding_now: 'Risk judgment, renewal planning, and account-priority calls still compound.',
      human_owned: 'Keep customer judgment, relationship context, and final account decisions human-owned.',
      business_result: 'Surface risk earlier and improve renewal-readiness without more manual prep.',
    },
    operations: {
      weakening_now: 'Manual follow-up, status aggregation, and low-value coordination work get cheaper first.',
      compounding_now: 'Exception handling, operating rules, and cross-functional prioritization still compound.',
      human_owned: 'Keep prioritization, exception logic, and stakeholder trade-offs human-owned.',
      business_result: 'Reduce coordination drag while improving throughput reliability.',
    },
    admin: {
      weakening_now: 'Calendar wrangling, note cleanup, and repetitive prep packaging get cheaper first.',
      compounding_now: 'Executive context, prioritization judgment, and decision-prep quality still compound.',
      human_owned: 'Keep context prioritization, executive nuance, and final briefing quality human-owned.',
      business_result: 'Reduce prep scramble while improving briefing quality and follow-through.',
    },
    product: {
      weakening_now: 'First-pass research synthesis, status packaging, and routine planning prep get cheaper first.',
      compounding_now: 'Decision judgment, prioritization logic, and cross-functional narrative quality still compound.',
      human_owned: 'Keep prioritization calls, trade-off judgment, and stakeholder alignment human-owned.',
      business_result: 'Shorten decision-prep cycles while improving planning clarity.',
    },
    general: {
      weakening_now: 'First-pass drafting, repetitive summaries, and routine coordination work get cheaper first.',
      compounding_now: 'Context judgment, review logic, and accountable decisions other people trust still compound.',
      human_owned: 'Keep review logic, quality control, and final decisions human-owned.',
      business_result: 'Improve speed or quality on one high-visibility workflow other people already care about.',
    },
  };
  const operatorFrame = operatorFrames[roleFamily] || operatorFrames.general;
  const proofTitle = stayAndAdvance?.thirty_day_plan?.proof_asset?.title || 'one internal AI leverage case';
  const managerAsk = stayAndAdvance?.promotion_conversation_pack?.ask
    || `Ask to own the next workflow adjacent to ${anchorTask.toLowerCase()} once the first case is working.`;

  return {
    headline: `Use AI in ${profile?.job_title || 'your role'} this week`,
    weakening_now: operatorFrame.weakening_now,
    compounding_now: operatorFrame.compounding_now,
    ...
    human_checkpoint: firstPlay.human_checkpoint || operatorFrame.human_owned,
    human_owned: operatorFrame.human_owned,
    output: firstPlay.what_to_share || proofTitle,
    business_result: firstPlay.business_impact || operatorFrame.business_result,
    ...
    manager_ask: managerAsk,
    next_adjacent_scope: secondaryTask || tertiaryTask || `${anchorTask} governance`,
  };
}
...
function buildEnglishStayRecommendation(roleFamily, goalNow) {
  ...
  if (roleFamily === 'legal') {
    return `${prompt} become the person who owns intake logic, review standards, and legal workflow quality instead of just pushing documents through the queue.`;
  }
  if (roleFamily === 'procurement') {
    return `${prompt} become the person who owns supplier judgment, sourcing communication, and the operating cadence behind better buying decisions.`;
  }
  if (roleFamily === 'education') {
    return `${prompt} become the person who owns learning-system quality, update speed, and the enablement workflows teams can actually reuse.`;
  }
  if (roleFamily === 'analytics') {
    return `${prompt} become the person who owns metric trust, KPI reviews, and the business readout leaders actually act on.`;
  }
  if (roleFamily === 'finance') {
    return `${prompt} become the person who turns planning cycles into faster scenario judgment, cleaner finance narratives, and reusable decision support.`;
  }
  if (roleFamily === 'hr') {
    return `${prompt} become the person who turns manager support, policy questions, and people insight into scalable operating systems.`;
  }
  if (roleFamily === 'admin') {
    return `${prompt} become the person who owns executive briefings, follow-through, and the operating rhythm leadership depends on.`;
  }
  if (roleFamily === 'customer') {
    return `${prompt} become the person who owns health signals, renewal judgment, and the customer workflows that keep revenue safer.`;
  }
  if (roleFamily === 'operations') {
    return `${prompt} become the person who redesigns intake, handoffs, and operating rhythm instead of carrying coordination by hand.`;
  }
  if (roleFamily === 'product') {
    return `${prompt} become the person who turns scattered decisions into repeatable planning systems and sharper prioritization.`;
  }
  if (roleFamily === 'marketing') {
    return `${prompt} become the person who owns experiment design, planning cadence, and growth reviews instead of just campaign throughput.`;
  }

  return goalNow === 'stay_and_advance'
    ? 'Stay in the same career lane and use AI to turn visible workflow leverage into broader scope, stronger protection, and promotion signal.'
    : 'Stay in the same career lane, but shift from raw execution into AI-directed judgment, workflow design, and visible operating leverage.';
}
...
function buildEnglishStayRationale(roleFamily, jobTitle, anchorTask, aiMaturity = '') {
  ...
  if (roleFamily === 'legal') {
    return `The better internal move is to make ${taskLabel} cleaner while owning intake rules, legal review logic, and reusable request standards other teams can trust.${aiReadyTail}`;
  }
  if (roleFamily === 'procurement') {
    return `The better internal move is to make ${taskLabel} faster while owning supplier comparison logic, sourcing readouts, and the decision cadence business partners rely on.${aiReadyTail}`;
  }
  if (roleFamily === 'education') {
    return `The stronger internal move is to make ${taskLabel} easier to ship while owning learning quality, update systems, and enablement workflows teams can reuse.${aiReadyTail}`;
  }
  if (roleFamily === 'analytics') {
    return `Your leverage is closer than a title change: make ${taskLabel} faster, then own the KPI definitions, review pack, and business narrative other teams already rely on.${aiReadyTail}`;
  }
  if (roleFamily === 'finance') {
    return `The durable move is to speed up ${taskLabel} while owning the planning workflow, scenario framing, and finance story leaders still need from ${jobTitle}.${aiReadyTail}`;
  }
  if (roleFamily === 'hr') {
    return `The stronger internal move is to make ${taskLabel} easier to deliver while building manager-support and workforce-planning systems other teams can actually reuse.${aiReadyTail}`;
  }
  if (roleFamily === 'admin') {
    return `The strongest internal move is to make ${taskLabel} cleaner while owning the briefing system, follow-through rhythm, and executive context around it.${aiReadyTail}`;
  }
  if (roleFamily === 'customer') {
    return `The smarter defensive move is to speed up ${taskLabel} while owning the health signals, risk reviews, and account playbooks the team uses to protect revenue.${aiReadyTail}`;
  }
  if (roleFamily === 'operations') {
    return `The advantage is not a bigger title yet. It is to make ${taskLabel} run with cleaner intake, clearer follow-through, and stronger team operating rules.${aiReadyTail}`;
  }
  if (roleFamily === 'product') {
    return `The better internal move is to make ${taskLabel} sharper while owning the decision system, prioritization logic, and review cadence around it.${aiReadyTail}`;
  }
  if (roleFamily === 'marketing') {
    return `The better move is to make ${taskLabel} faster while owning the planning system, test cadence, and growth review that turns marketing output into learning.${aiReadyTail}`;
  }

  return `The better internal move is to use AI to make ${taskLabel} faster, sharper, and more tightly connected to business decisions other people trust.${aiReadyTail}`;
}

function buildStayConfidenceRoleTail(roleFamily) {
  if (roleFamily === 'legal') {
    return ' In practice, that means owning contract intake quality, request-routing rules, and legal workflow standards other teams can actually follow.';
  }
  if (roleFamily === 'procurement') {
    return ' In practice, that means owning supplier review quality, sourcing readouts, and the procurement operating cadence stakeholders trust.';
  }
  if (roleFamily === 'education') {
    return ' In practice, that means owning learning-system quality, update workflows, and enablement standards teams can reuse.';
  }
  if (roleFamily === 'analytics') {
...
function buildRoleNativeStayWhyThisWon(roleFamily, strategyLed = false) {
  if (roleFamily === 'legal') {
    return strategyLed
      ? 'PivotIQ is leading with the move you can make real fastest: turn intake, review rules, and legal workflow quality into visible proof before forcing a broader title jump.'
      : 'PivotIQ is leading with the strongest current-lane advantage because legal workflow ownership, review standards, and visible request-quality proof are the fastest leverage from where you are now.';
  }
  if (roleFamily === 'procurement') {
    return strategyLed
      ? 'PivotIQ is leading with the move you can make real fastest: turn supplier reviews, sourcing communication, and procurement cadence into visible proof before forcing a bigger title jump.'
      : 'PivotIQ is leading with the strongest current-lane advantage because supplier judgment, sourcing readouts, and procurement operating proof are the fastest leverage from where you are now.';
  }
  if (roleFamily === 'education') {
    return strategyLed
      ? 'PivotIQ is leading with the move you can make real fastest: turn learning production, update workflows, and enablement quality into visible proof before forcing a broader title jump.'
      : 'PivotIQ is leading with the strongest current-lane advantage because learning-system quality, update speed, and reusable enablement proof are the fastest leverage from where you are now.';
  }
  if (roleFamily === 'customer') {
    return strategyLed
      ? 'PivotIQ is leading with the move you can make real fastest: tighten renewal signal quality, health reviews, and account playbooks before asking the market to trust a bigger title jump.'
      : 'PivotIQ is leading with the strongest current-lane advantage because better renewal judgment, account signal quality, and visible customer operating plays are the fastest proof from where you are now.';
  }
  if (roleFamily === 'operations') {
    return strategyLed
      ? 'PivotIQ is leading with the move you can make real fastest: redesign one workflow, tighten handoffs, and turn manual coordination into a visible operating system before forcing a bigger title jump.'
      : 'PivotIQ is leading with the strongest current-lane advantage because workflow ownership, cleaner handoffs, and visible operating proof are the fastest leverage from where you are now.';
  }
  if (roleFamily === 'finance') {
    return strategyLed
      ? 'PivotIQ is leading with the move you can make real fastest: turn planning work into faster scenario prep, decision-ready finance narratives, and visible business support before forcing a bigger title jump.'
      : 'PivotIQ is leading with the strongest current-lane advantage because scenario judgment, planning speed, and decision-ready finance proof are the fastest leverage from where you are now.';
  }
  if (roleFamily === 'analytics') {
    return strategyLed
      ? 'PivotIQ is leading with the move you can make real fastest: improve KPI review quality, dashboard trust, and decision-ready narratives before forcing a broader external title jump.'
      : 'PivotIQ is leading with the strongest current-lane advantage because cleaner KPI ownership, reporting trust, and business narrative proof are the fastest leverage from where you are now.';
  }
  if (roleFamily === 'marketing') {
    return strategyLed
      ? 'PivotIQ is leading with the move you can make real fastest: turn campaign work into clearer experiment design, planning systems, and growth review proof before forcing a bigger title jump.'
      : 'PivotIQ is leading with the strongest current-lane advantage because experiment design, campaign judgment, and planning-system proof are the fastest leverage from where you are now.';
  }
  if (roleFamily === 'hr') {
    return strategyLed
      ? 'PivotIQ is leading with the move you can make real fastest: turn manager support and people workflows into reusable systems before forcing a bigger external title jump.'
      : 'PivotIQ is leading with the strongest current-lane advantage because manager enablement, people-ops systems, and workforce-planning proof are the fastest leverage from where you are now.';
  }
  if (roleFamily === 'admin') {
    return strategyLed
      ? 'PivotIQ is leading with the move you can make real fastest: turn executive support into a reusable briefing and follow-through system before forcing a bigger external title jump.'
      : 'PivotIQ is leading with the strongest current-lane advantage because better executive context, cleaner follow-through, and visible operating proof are the fastest leverage from where you are now.';
  }
...
function buildRoleNativeStayNotYetReason(roleFamily, title, { strictSalaryTolerance = false, salaryRiskOnPivot = false, broadRoleThinSignal = false, lowConfidencePivot = false } = {}) {
  if (!title) return '';

  if (strictSalaryTolerance && salaryRiskOnPivot) {
    return `${title} is not the lead move yet because it asks for a riskier pay tradeoff before the market signal is strong enough to justify it.`;
  }

  if (broadRoleThinSignal) {
    if (roleFamily === 'customer') {
      return `${title} is not the lead move yet because the external signal is still too thin and role-mixed, while the cleaner first win is to turn renewal judgment and account reviews into visible proof.`;
    }
    if (roleFamily === 'operations') {
      return `${title} is not the lead move yet because the external signal is still too thin and role-mixed, while the cleaner first win is to turn workflow ownership and handoff design into visible operating proof.`;
    }
    if (roleFamily === 'finance') {
      return `${title} is not the lead move yet because the external signal is still too thin and role-mixed, while the cleaner first win is to turn planning and variance work into visible finance proof.`;
    }
    if (roleFamily === 'analytics') {
      return `${title} is not the lead move yet because the external signal is still too thin and role-mixed, while the cleaner first win is to turn KPI review and dashboard quality into visible proof.`;
    }
  }

  if (lowConfidencePivot) {
    return `${title} is not the lead move yet because it still reads like a stretch relative to the current evidence, proof, and market overlap.`;
  }

  if (roleFamily === 'customer') {
    return `${title} is not the lead move yet because PivotIQ still sees the cleaner advantage in proving customer judgment through renewal reviews, account signals, and reusable playbooks first.`;
  }
  if (roleFamily === 'operations') {
    return `${title} is not the lead move yet because PivotIQ still sees the cleaner advantage in proving workflow redesign, handoff ownership, and operating rhythm upgrades first.`;
  }
  if (roleFamily === 'finance') {
    return `${title} is not the lead move yet because PivotIQ still sees the cleaner advantage in proving finance judgment through scenario work, variance reviews, and decision-ready narratives first.`;
  }

  return `${title} is not the lead move yet because PivotIQ still sees the cleaner advantage in turning your current lane into visible leverage first.`;
}

function buildRoleNativeStayUnlockCondition(roleFamily, { strictSalaryTolerance = false, salaryRiskOnPivot = false, broadRoleThinSignal = false, proofStateLevel = 0 } = {}) {
  if (strictSalaryTolerance && salaryRiskOnPivot) {
    return 'The riskier pivot becomes more realistic once you can keep compensation risk tight and show one proof asset that clearly matches live postings.';
  }

  if (broadRoleThinSignal) {
    if (roleFamily === 'customer') {
      return 'The riskier pivot becomes more realistic once you have cleaner live-market overlap plus one proof asset that already looks like a renewal, account-health, or customer-operations review the target role would trust.';
    }
    if (roleFamily === 'operations') {
      return 'The riskier pivot becomes more realistic once you have cleaner live-market overlap plus one proof asset that already looks like a workflow redesign, handoff system, or operating review the target role would trust.';
    }
    if (roleFamily === 'finance') {
      return 'The riskier pivot becomes more realistic once you have cleaner live-market overlap plus one proof asset that already looks like finance planning, scenario review, or decision support the target role would trust.';
    }
    if (roleFamily === 'analytics') {
      return 'The riskier pivot becomes more realistic once you have cleaner live-market overlap plus one proof asset that already looks like KPI review, dashboard QA, or analytics decision support the target role would trust.';
    }
  }

  if (proofStateLevel < 2) {
    return 'The riskier pivot becomes more realistic once you have one visible proof asset and repeated job-posting overlap for the top requirements.';
  }

  return 'The riskier pivot becomes more realistic once live openings and your proof both point in the same direction.';
}
...
function confidenceSortWeight(confidenceState = '') {
  if (confidenceState === 'market-backed') return 3;
  if (confidenceState === 'strategy-led') return 2;
  if (confidenceState === 'low-confidence') return 1;
  return 0;
}

function titleLaneSignature(title = '') {
  return normalizeText(String(title || '')
    .replace(/\b(manager|lead|analyst|specialist|operations|strategy|director|coordinator|partner|head|senior|principal)\b/gi, ' ')
    .trim());
}

function pickConservativeBackupPivot(pivots = [], primaryPivotId = '', profile = null, primaryPivot = null) {
  const remaining = normalizeArray(pivots).filter((pivot) => pivot?.id !== primaryPivotId);
  if (!remaining.length) return null;
  const primarySignature = titleLaneSignature(primaryPivot?.title || '');

  return [...remaining].sort((left, right) => {
    const rightConfidence = confidenceSortWeight(buildPivotConfidenceState(right, profile).confidence_state);
    const leftConfidence = confidenceSortWeight(buildPivotConfidenceState(left, profile).confidence_state);
    if (rightConfidence !== leftConfidence) return rightConfidence - leftConfidence;

    const rightDistinct = titleLaneSignature(right?.title || '') !== primarySignature ? 1 : 0;
    const leftDistinct = titleLaneSignature(left?.title || '') !== primarySignature ? 1 : 0;
    if (rightDistinct !== leftDistinct) return rightDistinct - leftDistinct;
...
function buildRecommendationStack(reportData) {
  ...
  const backupPivot = shouldLeadWithStay
    ? primaryPivot
    : pickConservativeBackupPivot(pivots, primaryPivot?.id, profile, primaryPivot);
  ...
  const whyThisWon = primary?.type === 'stay'
    ? buildRoleNativeStayWhyThisWon(roleFamily, stayItem?.confidence_state === 'strategy-led')
    : primaryPivotConfidence.confidence_state === 'market-backed'
  ...
  const notYetReason = riskierAlternative
    ? primary?.type === 'stay'
      ? buildRoleNativeStayNotYetReason(roleFamily, riskierAlternative.title, {
          strictSalaryTolerance,
          salaryRiskOnPivot,
          broadRoleThinSignal,
          lowConfidencePivot: primaryPivotConfidence.confidence_state === 'low-confidence',
        })
  ...
  const unlockCondition = primary?.type === 'stay'
    ? buildRoleNativeStayUnlockCondition(roleFamily, {
        strictSalaryTolerance,
        salaryRiskOnPivot,
        broadRoleThinSignal,
        proofStateLevel,
...
function buildProofAssetExecutionGuide({ title, targetRole, primaryTask, topSkill, metricPrompt, internal }) {
  return {
    artifact_format: internal
      ? 'One-page operating case with one before/after workflow, one metric snapshot, and one next-scope ask.'
      : 'Compact case study with one before/after workflow, one metric snapshot, and one hiring-manager readout.',
    inputs_to_collect: [
      `One real example from ${primaryTask.toLowerCase()}`,
      `One review checkpoint showing where ${topSkill.toLowerCase()} stayed under human control`,
      `One metric or decision signal tied to ${metricPrompt.toLowerCase()}`,
    ],
    one_week_ship: internal
      ? `By the end of the week, ship a manager-readable case showing how ${primaryTask.toLowerCase()} now runs better and why that points toward ${targetRole}.`
      : `By the end of the week, ship a clean proof asset showing how ${topSkill.toLowerCase()} improved ${primaryTask.toLowerCase()} in a way that reads like ${targetRole}.`,
    good_enough_bar: internal
      ? 'A manager can skim it in under three minutes, see one business improvement, and understand what broader scope you should own next.'
      : 'A hiring manager can skim it in under three minutes, see one real business outcome, and understand why you already operate like the target role.',
    manager_readout: internal
      ? `I redesigned ${primaryTask.toLowerCase()} so AI handles the first pass while I keep the review logic and business judgment. This case shows the workflow change, the metric movement, and the broader scope I can now own.`
      : `This case shows how I used ${topSkill.toLowerCase()} to redesign ${primaryTask.toLowerCase()} with clear review controls and a measurable business result.`,
    resume_bullet_formula: internal
      ? `Redesigned ${primaryTask.toLowerCase()} with AI-assisted workflow controls, improving ${metricPrompt.toLowerCase()} while creating a repeatable operating pattern for the team.`
      : `Built a proof asset for ${targetRole} by redesigning ${primaryTask.toLowerCase()} with ${topSkill.toLowerCase()}, improving ${metricPrompt.toLowerCase()} with clear review controls.`,
  };
}

function buildStayJobSafetyCase(reportData) {
  const stayAndAdvance = reportData?.stay_and_advance || {};
  const profile = reportData?.profile || {};
  const roleFamily = inferRoleFamilyFromProfile(
    profile?.job_title || profile?.jobTitle || '',
  ...
  const primaryTask = normalizeArray(profile?.primary_tasks)[0] || normalizeArray(profile?.tasks)[0] || 'your most visible workflow';
  const metric = stayAndAdvance?.thirty_day_plan?.metric_to_move || 'one visible operating metric';
  const nextTitle = stayAndAdvance?.promotion_path?.next_title || 'the stronger version of your current role';

  const roleSpecificReasons = {
    legal: [
      'You stop competing on request cleanup and document shuffling alone and start owning legal workflow quality, routing rules, and review standards.',
      'Your value shifts from moving work through the queue to making legal intake and review more scalable and trustworthy.',
      'That makes you harder to replace because the scarce layer is not first-pass summarization, it is risk judgment and workflow control.',
    ],
    procurement: [
      'You stop competing on comparison formatting and follow-up work alone and start owning supplier judgment, sourcing communication, and decision cadence.',
      'Your value shifts from process handling to helping the business make cleaner buying decisions faster.',
      'That makes you safer because AI can accelerate sourcing prep, but teams still need someone who can frame trade-offs and recommendations.',
    ],
    education: [
      'You stop competing on manual content churn alone and start owning learning-system quality, update workflows, and enablement standards.',
      'Your value shifts from shipping assets to making programs easier to update, trust, and reuse.',
      'That makes you safer because the scarce layer is not first-draft content, it is audience judgment and system design.',
    ],
    marketing: [
      'You stop competing with AI on raw content volume and start owning experiment judgment, review standards, and campaign operating patterns.',
      'Your value shifts from shipping more assets to designing the system that makes campaigns faster and more trustworthy.',
      'Leadership can connect the work to pipeline, conversion, and learning speed instead of private productivity.',
    ],
    analytics: [
      'You become the QA and decision layer between raw analysis and leadership action.',
      'Your value shifts from dashboard output to metric interpretation, anomaly review, and narrative clarity.',
      'That makes you harder to replace because teams still need someone who can challenge bad signals and frame the right next move.',
    ],
    finance: [
      'You stop competing on spreadsheet throughput and start owning planning judgment, scenario framing, and recommendation quality.',
      'Your value shifts from producing numbers to helping leadership trust faster decisions.',
      'That protects your role because the scarce layer is not formatting the model, it is controlling the decision logic around it.',
    ],
    hr: [
      'You become the judgment layer for manager support, policy interpretation, and people-risk handling.',
      'Your value shifts from reactive support volume to operating design leaders can trust.',
      'That makes you safer because sensitive people decisions still need context, exception handling, and credibility.',
    ],
    customer: [
      'You stop competing on note-taking and coordination alone and start owning risk review, renewal judgment, and customer operating patterns.',
      'Your value shifts from reactive account support to proactive commercial signal and workflow design.',
      'That makes you harder to cut because you become the person who turns messy customer context into action.',
    ],
    operations: [
      'You stop competing on manual coordination volume and start owning intake design, exception logic, and workflow governance.',
      'Your value shifts from chasing status updates to making the system itself run better.',
      'That makes you safer because AI can assist throughput, but teams still need someone to define the controls and operating pattern.',
    ],
    admin: [
      'You stop competing on invisible coordination and start owning executive context, decision prep, and repeatable operating rhythm.',
      'Your value shifts from handling requests to shaping how leadership work gets organized and trusted.',
      'That makes you harder to replace because the scarce layer is context judgment, not just task completion.',
    ],
    general: [
      'You stop competing with AI on first-draft output and start owning review judgment, workflow design, and visible business leverage.',
      'Your value shifts from doing more tasks to making the whole workflow faster and safer for other people.',
      'That makes you safer because AI compresses execution first, not the person who controls the operating pattern.',
    ],
  };

  const saferBecause = roleSpecificReasons[roleFamily] || roleSpecificReasons.general;
  const roleSpecificSummaries = {
    legal: `This path makes you safer by moving you from request throughput into legal workflow quality, review standards, and the intake logic the business can trust.`,
    procurement: `This path makes you safer by moving you from sourcing admin and formatting into supplier judgment, clearer buying decisions, and a stronger procurement operating cadence.`,
    education: `This path makes you safer by moving you from content churn into learning-system quality, update speed, and enablement workflows teams can reuse.`,
    marketing: `This path makes you safer by moving you from campaign throughput into experiment judgment, growth review, and the marketing system leaders trust.`,
    analytics: `This path makes you safer by moving you from dashboard output into metric trust, anomaly review, and the final business readout teams rely on.`,
    finance: `This path makes you safer by moving you from spreadsheet throughput into planning judgment, scenario control, and the finance narrative leadership uses.`,
    hr: `This path makes you safer by moving you from reactive support into manager-system design, people-risk judgment, and reusable operating patterns.`,
    customer: `This path makes you safer by moving you from reactive account coordination into revenue-signal judgment, renewal control, and reusable customer playbooks.`,
    operations: `This path makes you safer by moving you from manual coordination into intake logic, exception control, and operating rules the team can repeat.`,
    admin: `This path makes you safer by moving you from invisible support labor into executive context, decision prep, and a repeatable leadership cadence.`,
    general: `This path makes you safer by moving you from raw execution into visible judgment, workflow ownership, and operating leverage other people depend on.`,
  };

  return {
    headline: 'Why this makes you safer in the AI shift',
    summary: roleSpecificSummaries[roleFamily] || `This path makes you safer by turning ${primaryTask.toLowerCase()} into visible judgment, workflow ownership, and reusable leverage.`,
    safer_because: saferBecause,
    what_changes_if_you_do_this: [
      `Your work starts reading like ${nextTitle}, not just your current title.`,
      `Leadership can connect the workflow change to ${metric.toLowerCase()}.`,
      'You create a visible layer of judgment, quality control, and operating leverage other people can reuse.',
    ],
    if_you_ignore_this: `If you keep competing on raw execution alone, AI is more likely to compress the part of the role you are defending instead of strengthening it.`,
    metric_to_watch: metric,
  };
}

function buildPromotionCase(reportData) {
  const stayAndAdvance = reportData?.stay_and_advance || {};
  const profile = reportData?.profile || {};
  const primaryTask = normalizeArray(profile?.primary_tasks)[0] || normalizeArray(profile?.tasks)[0] || 'one visible workflow';
  const targetTitle = stayAndAdvance?.promotion_path?.next_title || 'the stronger version of your current role';
  const proofAssetTitle = stayAndAdvance?.thirty_day_plan?.proof_asset?.title || 'your internal AI leverage case';
  const metric = stayAndAdvance?.thirty_day_plan?.metric_to_move || 'one operating metric';
  const leadershipNarrative = stayAndAdvance?.thirty_day_plan?.leadership_narrative || `I am using AI to make ${primaryTask.toLowerCase()} more reliable and more strategic.`;

  return {
    headline: `How this turns into ${targetTitle}`,
    target_title: targetTitle,
    why_now: `Promotion case strength comes from showing that you can redesign ${primaryTask.toLowerCase()} for the team, not just use AI privately for yourself.`,
    leadership_case: [
      `Show the before/after operating change around ${primaryTask.toLowerCase()}.`,
      `Tie the improvement to ${metric.toLowerCase()} so the case reads in business language.`,
      `Use ${proofAssetTitle.toLowerCase()} as evidence that you can now own the next layer of scope.`,
    ],
    proof_to_show: [
      proofAssetTitle,
      `One metric snapshot tied to ${metric.toLowerCase()}`,
      'One workflow map or review standard another teammate could reuse',
    ],
    manager_sentence: leadershipNarrative,
    what_changes_if_yes: `If this lands well, you stop being evaluated only on output volume and start being trusted with broader workflow, process, or decision ownership toward ${targetTitle}.`,
  };
}
...
function buildProofAssetBuilder(reportData) {
  ...
  const builder = {
    ...
    share_prompt_label: 'Use this in outreach',
    proof_state_read: proofAction,
    execution_guide: buildProofAssetExecutionGuide({
      title,
      targetRole,
      primaryTask,
      topSkill,
...
function buildStayProofAssetBuilder(reportData) {
  ...
  return {
    ...
    first_action: firstAction,
    share_prompt: sharePrompt,
    share_prompt_label: 'Use this with your manager',
    proof_state_read: proofAction,
    execution_guide: buildProofAssetExecutionGuide({
      title,
      targetRole,
      primaryTask,
      topSkill: 'AI-assisted workflow judgment',
      metricPrompt: metric,
      internal: true,
    }),
    internal_version: {
    ...
    finance: [
      {
        ...
        business_impact: 'Finance spends less time formatting planning work and more time shaping decision-ready scenario calls.',
...
function addPaidValueFields(reportData) {
  ...
  const withBuilder = {
    ...
    pivots: normalizeArray(reportData.pivots).map((pivot) => ({
      ...pivot,
      learning_path: buildLearningPath(pivot?.skill_gaps, reportData?.profile || null, 'pivot'),
    })),
    stay_path: reportData.stay_path && typeof reportData.stay_path === 'object'
      ? {
          ...reportData.stay_path,
          learning_path: buildLearningPath(reportData.stay_path?.skill_gaps, reportData?.profile || null, 'stay'),
        }
      : reportData.stay_path,
    stay_and_advance: reportData.stay_and_advance && typeof reportData.stay_and_advance === 'object'
      ? {
          ...
          job_safety_case: reportData.stay_and_advance.job_safety_case && typeof reportData.stay_and_advance.job_safety_case === 'object'
            ? { ...reportData.stay_and_advance.job_safety_case, ...buildStayJobSafetyCase(reportData) }
            : buildStayJobSafetyCase(reportData),
          promotion_case: reportData.stay_and_advance.promotion_case && typeof reportData.stay_and_advance.promotion_case === 'object'
            ? { ...reportData.stay_and_advance.promotion_case, ...buildPromotionCase(reportData) }
            : buildPromotionCase(reportData),
...
function buildStayAndAdvance(profile, summary, bestPivot, locale = 'en') {
  ...
  return {
    ...
    thirty_day_plan: {
      this_week: [
        locale === 'nl' ? `Kies één workflow, te beginnen met ${anchorTask.toLowerCase()}, en herbouw die zodat AI de eerste versie doet en jij de reviewlogica houdt.` : locale === 'de' ? `Wähle einen Workflow, beginnend mit ${anchorTask.toLowerCase()}, und baue ihn so um, dass KI den ersten Pass übernimmt und du die Review-Logik behältst.` : `Choose one workflow, starting with ${anchorTask.toLowerCase()}, and redesign it so AI handles the first pass while you keep the review logic.`,
        locale === 'nl' ? 'Leg vast wat AI aanraakt, wat menselijk eigendom blijft, en welke metric, kwaliteitsverbetering of besluitversnelling hierdoor moet bewegen.' : locale === 'de' ? 'Halte fest, was KI berührt, was menschlich geführt bleibt und welche Kennzahl, Qualitätsverbesserung oder Entscheidungsbeschleunigung sich dadurch bewegen soll.' : 'Document what AI touches, what stays human-owned, and which metric, quality gain, or decision-speed result should move.',
      ],
      this_month: [
        locale === 'nl' ? 'Lever één manager-leesbare voor-na case op waarin de workflow, controlepunten en zichtbare impact duidelijk zijn.' : locale === 'de' ? 'Liefere einen manager-lesbaren Vorher-Nachher-Case, in dem Workflow, Kontrollpunkte und sichtbare Wirkung klar sind.' : 'Ship one manager-readable before/after case that makes the workflow, control points, and visible impact obvious.',
        locale === 'nl' ? 'Gebruik die case om bredere scope, eigenaarschap of een volgende pilot te vragen.' : locale === 'de' ? 'Nutze diesen Case, um breiteren Scope, Ownership oder einen nächsten Piloten zu beantragen.' : 'Use that case to ask for broader scope, ownership, or the next adjacent pilot.',
      ],
...
function buildStayAdvanceSkillGaps(profile, stayAndAdvance, basePivot, locale = 'en') {
  ...
  const stackAwareGap = buildStaySystemSkillGap({
    profile,
    roleFamily,
    primaryTask,
    fallbackResource: resources.workflow,
  });
  const primarySystem = getSortedRoleSystems(profile, roleFamily)[0] || '';
  const adjacentHardSkillGap = buildAdjacentStayHardSkillGap({
    profile,
    roleFamily,
    primaryTask,
    primarySystem,
  ...
  const staySkillGaps = stackAwareGap
    ? [
        stackAwareGap,
        ...(adjacentHardSkillGap ? [adjacentHardSkillGap] : [buildSkillGap({
    ...
    : [
        ...
        ...(adjacentHardSkillGap ? [adjacentHardSkillGap] : [buildSkillGap({
          ...
          skillType: 'decision_communication',
  ...
  return dedupeSkillGaps(staySkillGaps);
}
...
export function normalizeReportData(reportData, fallbackProfile = null) {
  ...
  const stayPreview = buildStayAndAdvance(profile, summary, defaultPivot, locale);
  pivots = dedupePivotsByTitle(pivots);
  const resolvedPivot = pivots[0] || defaultPivot || null;
  const decision = reportData.decision && typeof reportData.decision === 'object'
    ? { ...buildDecisionSignal(summary, resolvedPivot), ...reportData.decision }
    : buildDecisionSignal(summary, resolvedPivot);
  const nextMove = buildNextMove(profile, resolvedPivot, locale);
  const careerRoi = reportData.career_roi && typeof reportData.career_roi === 'object'
    ? { ...buildCareerRoi(summary, resolvedPivot, profile), ...reportData.career_roi }
    : buildCareerRoi(summary, resolvedPivot, profile);
  const stayAndAdvanceBase = stayPreview && resolvedPivot?.title === (pivots[0]?.title || '')
    ? stayPreview
    : buildStayAndAdvance(profile, summary, resolvedPivot, locale);
  const stayAndAdvance = reportData.stay_and_advance && typeof reportData.stay_and_advance === 'object'
    ? { ...stayAndAdvanceBase, ...reportData.stay_and_advance }
    : stayAndAdvanceBase;
  const stayPath = buildStayAdvancePath(profile, summary, stayAndAdvance, resolvedPivot, locale);
  const first30Days = buildFirst30Days(profile, resolvedPivot, { weeks: resolvedPivot?.roadmap?.weeks || [] }, nextMove);
  const dedupedTopLevelSkillGaps = dedupeSkillGaps(
    Array.isArray(reportData.skill_gaps)
      ? reportData.skill_gaps
      : resolvedPivot?.skill_gaps || []
  );
  const dedupedStayPathSkillGaps = dedupeSkillGaps(Array.isArray(stayPath?.skill_gaps) ? stayPath.skill_gaps : []);
  ...
  const normalizedReport = {
    ...
    pivots: pivots.map((pivot) => ({
      ...(() => {
        const dedupedPivotSkillGaps = dedupeSkillGaps(Array.isArray(pivot.skill_gaps) ? pivot.skill_gaps : []);
        return {
      ...pivot,
    ...
    stay_path: {
      ...
      skill_gaps: dedupedStayPathSkillGaps,
      learning_path: Array.isArray(stayPath?.learning_path) && stayPath.learning_path.length
        ? buildLearningPath(dedupedStayPathSkillGaps, profile, 'stay')
        : buildLearningPath(dedupedStayPathSkillGaps, profile, 'stay'),
      roadmap: {
