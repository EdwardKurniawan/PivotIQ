...
function buildEnglishStayRationale(roleFamily, jobTitle, anchorTask, aiMaturity = '') {
  ...
  if (roleFamily === 'analytics') {
    return `The safer move is not to outrun ${jobTitle}. It is to make ${taskLabel} faster while owning the KPI definitions, review pack, and business narrative other teams depend on.${aiReadyTail}`;
  }
  if (roleFamily === 'finance') {
    return `The safer move is not to outrun ${jobTitle}. It is to make ${taskLabel} faster while owning the planning workflow, scenario framing, and decision story leaders still need from finance.${aiReadyTail}`;
  }
  if (roleFamily === 'hr') {
    return `The safer move is not to outrun ${jobTitle}. It is to make ${taskLabel} faster while building manager-support and workforce-planning systems other teams can reuse.${aiReadyTail}`;
  }
  if (roleFamily === 'admin') {
    return `The safer move is not to outrun ${jobTitle}. It is to make ${taskLabel} faster while owning the briefing system, follow-up rhythm, and executive context around it.${aiReadyTail}`;
  }
  if (roleFamily === 'customer') {
    return `The safer move is not to outrun ${jobTitle}. It is to make ${taskLabel} faster while owning the health signals, risk reviews, and account playbooks the team uses.${aiReadyTail}`;
  }
  if (roleFamily === 'operations') {
    return `The safer move is not to outrun ${jobTitle}. It is to make ${taskLabel} faster while redesigning the intake, follow-through, and team rhythms behind it.${aiReadyTail}`;
  }
  if (roleFamily === 'product') {
    return `The safer move is not to outrun ${jobTitle}. It is to make ${taskLabel} faster while owning the decision system, prioritization logic, and review cadence around it.${aiReadyTail}`;
  }
  if (roleFamily === 'marketing') {
    return `The safer move is not to outrun ${jobTitle}. It is to make ${taskLabel} faster while owning the planning system, test cadence, and growth review around it.${aiReadyTail}`;
  }

  return `The safer move is not to outrun ${jobTitle}. It is to use AI to make ${taskLabel} faster, sharper, and more tightly connected to business decisions other people trust.${aiReadyTail}`;
}
...
function buildStayJobSafetyCase(reportData) {
  ...
  return {
    ...
    summary: `The goal is not to look “good at AI.” The goal is to become the person who makes ${primaryTask.toLowerCase()} faster, safer, and more decision-ready in a way the team depends on.`,
