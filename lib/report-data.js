...
function buildStayAndAdvance(profile, summary, bestPivot, locale = 'en') {
  ...
  const localizedLeverageOps = [
    {
      title: locale === 'nl' ? 'Maak repetitieve output lichter' : locale === 'de' ? 'Mache repetitive Outputs leichter' : 'Make repetitive output lighter',
    ...
    {
      title: locale === 'nl' ? 'Verplaats jezelf richting interpretatie' : locale === 'de' ? 'Verschiebe dich Richtung Interpretation' : 'Move yourself toward interpretation',
    ...
    {
      title: locale === 'nl' ? 'Word de AI-adoptielaag in je team' : locale === 'de' ? 'Werde die KI-Adoptionsschicht in deinem Team' : 'Become the AI adoption layer for your team',
  ...
  return {
    ...
    promotion_path: {
      next_title: bestPivot?.title || (locale === 'nl' ? 'Sterkere variant van je huidige rol' : locale === 'de' ? 'Stärkere Version deiner aktuellen Rolle' : 'Stronger version of your current role'),
    ...
    thirty_day_plan: {
      ...
      metric_to_move: locale === 'nl'
        ? 'Kies één metric: doorlooptijd, revisies, foutpercentage of briefingkwaliteit.'
        : locale === 'de'
          ? 'Wähle eine Kennzahl: Durchlaufzeit, Revisionen, Fehlerquote oder Briefing-Qualität.'
          : 'Choose one metric to move: turnaround time, revisions, error rate, or briefing quality.',
      leadership_narrative: locale === 'nl'
        ? `Positioneer dit als: "Ik gebruik AI niet alleen om sneller te werken, maar om ${jobTitle.toLowerCase()} betrouwbaarder en strategischer te maken."`
        : locale === 'de'
          ? `Positioniere es so: "Ich nutze KI nicht nur, um schneller zu arbeiten, sondern um ${jobTitle.toLowerCase()} verlässlicher und strategischer zu machen."`
          : `Position this as: "I am not just using AI to work faster. I am using it to make ${jobTitle.toLowerCase()} more reliable and more strategic."`,
      proof_asset: {
        title: locale === 'nl' ? 'Interne AI-leverage case' : locale === 'de' ? 'Interner KI-Leverage-Case' : 'Internal AI leverage case',
