  ...
  if (openings >= 1 || fitScore >= 20 || matchScore >= 78) {
    return {
      ...
      confidence_reason: openings
        ? `${openings} live opening${openings === 1 ? '' : 's'} support the direction, but you still need clearer proof before this becomes a high-confidence move.${experienceNote}${technicalNote}${locationNote}`
        : `The title is directionally strong for your background, but the current market signal is still too thin to treat it as fully validated.${experienceNote}${technicalNote}${locationNote}`,
    };
  }
...
function buildRecommendationStack(reportData) {
  ...
  return {
    ...
    decision_brief: {
      ...
      summary: `${decisionBriefSummary}${salaryContext}${proofContext}`,
