...
function buildLeveragePivotTitle(roleFamily, jobTitle, industry) {
  ...
  if (roleFamily === 'operations') return 'Business Process Analyst';
...
function buildFastCashPivotTitle(roleFamily, jobTitle, industry) {
  ...
  if (roleFamily === 'finance') return 'Finance Transformation Lead';
  if (roleFamily === 'hr') return 'Manager Enablement Lead';
  if (roleFamily === 'customer') return 'Customer Enablement Lead';
  if (roleFamily === 'operations') return 'Operations Enablement Lead';
...
function buildHighUpsidePivotTitle(roleFamily, jobTitle, industry) {
  if (roleFamily === 'legal') {
    return chooseDistinctTitle(
      ['Legal Technology Strategy Director', 'Director of Legal Innovation', 'Legal Data Strategy Director'],
      jobTitle,
      'Legal Technology Strategy Director'
    );
  }
  if (roleFamily === 'procurement') {
    return chooseDistinctTitle(
  ...
  if (roleFamily === 'education') {
    return chooseDistinctTitle(
      ['Director of Learning Strategy', 'Director of Customer Education', 'Learning Experience Director'],
      jobTitle,
...
function buildLongTermPivotTitle(roleFamily, jobTitle, industry) {
  if (roleFamily === 'legal') {
    return chooseDistinctTitle(
      ['Contract Lifecycle Automation Architect', 'Legal Workflow Automation Architect', 'Compliance Systems Architect'],
      jobTitle,
      'Contract Lifecycle Automation Architect'
    );
  }
  if (roleFamily === 'procurement') {
    return chooseDistinctTitle(
      ['Procurement Automation Architect', 'Sourcing Systems Architect', 'Sustainable Sourcing Strategist'],
      jobTitle,
      'Procurement Automation Architect'
    );
  }
  if (roleFamily === 'education') {
    return chooseDistinctTitle(
      ['Learning Systems Architect', 'Enablement Systems Strategist', 'AI Learning Platform Lead'],
      jobTitle,
...
function buildPaidValueSummary(reportData) {
  const pivot = normalizeArray(reportData?.pivots)[0] || {};
  const first30Days = reportData?.first_30_days || {};
  const proofAsset = first30Days?.proof_asset?.title || `${pivot.title || 'Target role'} proof asset`;

  return {
    headline: `Your best move: ${pivot.title || 'build a more resilient next role'}`,
    recommended_move: reportData?.decision?.headline || `Move toward ${pivot.title || 'the strongest pivot path'}`,
    why_this_move: pivot.ranking_reason || pivot.fit_summary || reportData?.decision?.rationale || '',
    confidence_label: reportData?.decision?.confidence_label || (pivot.live_market_signal?.profile_fit_score ? 'Market-informed' : 'Model-led, verify with postings'),
    market_evidence: summarizeMarketEvidence(pivot),
    first_proof_asset: proofAsset,
    first_learning_step: getFirstLearningStep(pivot),
...
function addPaidValueFields(reportData) {
  ...
  return {
    ...withBuilder,
    paid_value_summary: withBuilder.paid_value_summary && typeof withBuilder.paid_value_summary === 'object'
      ? { ...withBuilder.paid_value_summary, ...buildPaidValueSummary(withBuilder) }
      : buildPaidValueSummary(withBuilder),
  };
}
