...
function pickStartingSkillGap(skillGaps = [], profile = null) {
  ...
  const defaultCritical = withResources.find((skill) => normalizeText(skill?.gap_priority) === 'critical') || withResources[0];
  if (!hasAdvancedAiMaturity(profile)) {
    return defaultCritical;
  }

  return withResources.find((skill) =>
    normalizeText(skill?.gap_priority) === 'critical'
    && isAdvancedAiWorkflowSkill(skill)
    && !isBeginnerAiSkill(skill)
  ) || withResources.find((skill) =>
    isAdvancedAiWorkflowSkill(skill)
    && !isBeginnerAiSkill(skill)
  ) || defaultCritical;
}
...
function buildPivotConfidenceState(pivot, profile = null) {
  ...
  if (openings >= 5 && fitScore >= 35 && missingCount <= 3 && overshoot === 0 && !(marketCaution && openings < 7)) {
    return {
  ...
  if (openings >= 1 || fitScore >= 20 || matchScore >= 78) {
    return {
      ...
      confidence_reason: openings
        ? `${openings} live opening${openings === 1 ? '' : 's'} support the direction, but you still need clearer proof before this becomes a high-confidence move.${experienceNote}${locationNote}`
        : `The title is directionally strong for your background, but the current market signal is still too thin to treat it as fully validated.${experienceNote}${locationNote}`,
    };
  }
  ...
  return {
    ...
    confidence_reason: modelOnlyCount >= 3
      ? `Several of the remaining gaps still come mostly from model logic rather than repeated live-market demand, so this should be treated as a hypothesis, not a bet.${experienceNote}${locationNote}`
      : `This title is adjacent, but there is not enough clean market evidence yet to rely on it as the main move.${experienceNote}${locationNote}`,
  };
}
...
function buildStayConfidenceState(summary, stayAndAdvance, profile = null) {
  ...
  if (goalNow === 'stay_and_advance' && score < 80) {
    return {
      ...
      confidence_reason: `You explicitly want the strongest move inside your current lane, and the current evidence still supports using AI to move toward ${nextTitle} faster than forcing a bigger reset.${aiReadyTail}`,
    };
  }
  ...
  if (score < 45) {
    return {
      ...
      confidence_reason: `The role is not under immediate collapse, so the clearest win is to use AI to move toward ${nextTitle} before you spend energy on a harder external transition.${aiReadyTail}`,
    };
  }
  ...
  if (score < 65) {
    return {
      ...
      confidence_reason: `You still have enough leverage inside the current role to turn AI into broader scope, better visibility, and a stronger internal promotion case toward ${nextTitle}.${aiReadyTail}`,
    };
  }
  ...
  return {
    ...
    confidence_reason: `Pressure is rising in the role, so staying only works if you quickly turn AI usage into visible scope, proof, and promotion signal while validating outside options in parallel.${aiReadyTail}`,
  };
}
...
function buildRecommendationStack(reportData) {
  ...
  if (stayItem && goalNow === 'stay_and_advance') {
    ...
    shouldLeadWithStay = shouldLeadWithStay || (primaryPivotConfidence.confidence_state !== 'market-backed' && riskScore < 72);
  } else if (stayItem && goalNow === 'active_pivot') {
    shouldLeadWithStay = primaryPivotConfidence.confidence_state === 'low-confidence' && riskScore < 78;
  } else if (stayItem && goalNow === 'not_sure') {
    shouldLeadWithStay = shouldLeadWithStay || (timelineUrgency === 'exploring_only' && primaryPivotConfidence.confidence_state !== 'market-backed');
  }
...
function buildStayAndAdvance(profile, summary, bestPivot, locale = 'en') {
  ...
  const localizedRationale = locale === 'nl'
    ...
    : locale === 'de'
      ...
      : `${jobTitle} is not automatically a leave signal. The strongest internal move is often to use AI to make ${anchorTask.toLowerCase()} faster, sharper, and more tightly connected to business decisions.${aiMaturityRank(aiMaturity) >= 2 ? ' You already have enough AI familiarity to make that visible quickly.' : ''}`;
