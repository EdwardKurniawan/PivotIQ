...
export default function LocalFullIntakePage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (reportData.generation_stage === 'full_complete') {
        router.replace('/report');
        return;
      }
      ...
      console.error('Failed to load stored report for full intake', error);
    }
  }, [router]);

  if (payload?.reportData) {
    return <FullReportIntake payload={payload} uiLocale={payload.uiLocale || locale} backHref="/report" />;
  }
