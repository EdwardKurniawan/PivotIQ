...
function buildReminderEmail(type, report) {
  ...
  if (type === 'celebration') {
    return {
      ...
      html: `<p>You completed Week ${targetWeek} of your PivotIQ roadmap for ${jobTitle} in ${industry}. Keep the momentum going and review your next milestone in the dashboard.</p>`,
    };
  }
  ...
  if (type === 'at_risk') {
    return {
      ...
      html: `<p>Your Week ${targetWeek} milestone for ${jobTitle} is behind schedule. Open your dashboard to review the catch-up plan and get back on track.</p>`,
    };
  }

  return {
    subject: `Week ${targetWeek} is up next in your PivotIQ roadmap`,
    html: `<p>Your next PivotIQ milestone is ready. Open your dashboard, review Week ${targetWeek}, and block time for the actions that matter most.</p>`,
  };
}
...
export async function POST(request) {
  try {
    ...
    const { data: dueReminders, error } = await supabase
      ...
      .select('id, user_email, type, current_week, job_title, industry')
      .eq('status', 'pending')
